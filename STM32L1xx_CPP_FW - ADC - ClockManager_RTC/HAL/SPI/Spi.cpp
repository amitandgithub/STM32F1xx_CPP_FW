...
//        m_Transaction.Spi_Baudrate = SPI_BAUDRATE_DIV2;
    ...
    {     
      ...
      {   
        DigitalIO::SetOutputMode(ClkPort,ClkPin,OUTPUT_AF_PUSH_PULL,LL_GPIO_SPEED_FREQ_VERY_HIGH); // sck.HwInit(OUTPUT_AF_PUSH_PULL);
        //DigitalIO::SetInputMode(MisoPort,MisoPin,INPUT_PULLUP); //miso.HwInit(INPUT_PULLUP);
        DigitalIO::SetOutputMode(MisoPort,MisoPin,OUTPUT_AF_PUSH_PULL,LL_GPIO_SPEED_FREQ_VERY_HIGH); //miso.HwInit(INPUT_PULLUP);
        DigitalIO::SetOutputMode(MosiPort,MosiPin,OUTPUT_AF_PUSH_PULL,LL_GPIO_SPEED_FREQ_VERY_HIGH); // sck.HwInit(OUTPUT_AF_PUSH_PULL);
        DigitalIO::SetAlternateFunction(ClkPort,ClkPin,LL_GPIO_AF_5);
        DigitalIO::SetAlternateFunction(MisoPort,MisoPin,LL_GPIO_AF_5);
        DigitalIO::SetAlternateFunction(MosiPort,MosiPin,LL_GPIO_AF_5);
      }
      ...
      {    
        // Check if all GPIOs in mode OUTPUT_AF_PUSH_PULL work in Slave mode
        ...
        DigitalIO::SetOutputMode(MisoPort,MisoPin,OUTPUT_AF_PUSH_PULL,LL_GPIO_SPEED_FREQ_VERY_HIGH); // miso(MisoPort,MisoPin);
        DigitalIO::SetInputMode(MosiPort,MosiPin,INPUT_PULLDOWN); //mosi(MosiPort,MosiPin);
        DigitalIO::SetAlternateFunction(ChipSelectPort,ChipSelectPin,LL_GPIO_AF_5);
        DigitalIO::SetAlternateFunction(ClkPort,ClkPin,LL_GPIO_AF_5);
        DigitalIO::SetAlternateFunction(MisoPort,MisoPin,LL_GPIO_AF_5);
        DigitalIO::SetAlternateFunction(MosiPort,MosiPin,LL_GPIO_AF_5);

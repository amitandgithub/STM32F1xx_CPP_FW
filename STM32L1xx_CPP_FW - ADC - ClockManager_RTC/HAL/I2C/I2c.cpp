...
namespace HAL
{
  ...
  I2c::I2c(I2cPort_t I2cPort) :
    m_I2CState(I2C_RESET)
#if (I2C_MASTER_DMA == 1)
      ,m_I2C_DMA_Rx_Callback(this),
      m_I2C_DMA_Tx_Callback(this)
#endif
      {
    ...
    I2c::I2CStatus_t I2c::HwInit(uint32_t Hz)
    {        	
      ...
      if(m_I2Cx == I2C1)
      {
        DigitalIO::SetOutputMode(B6,OUTPUT_AF_OPEN_DRAIN,LL_GPIO_SPEED_FREQ_HIGH); // igitalOut<B6,OUTPUT_AF_OPEN_DRAIN> scl;
        DigitalIO::SetOutputMode(B7,OUTPUT_AF_OPEN_DRAIN,LL_GPIO_SPEED_FREQ_HIGH); // DigitalOut<B7,OUTPUT_AF_OPEN_DRAIN> sda;
        DigitalIO::SetAlternateFunction(B6,LL_GPIO_AF_4);
        DigitalIO::SetAlternateFunction(B7,LL_GPIO_AF_4);
      }
      else if(m_I2Cx == I2C2)
      {
        DigitalIO::SetOutputMode(B10,OUTPUT_AF_OPEN_DRAIN,LL_GPIO_SPEED_FREQ_HIGH); // igitalOut<B6,OUTPUT_AF_OPEN_DRAIN> scl;
        DigitalIO::SetOutputMode(B11,OUTPUT_AF_OPEN_DRAIN,LL_GPIO_SPEED_FREQ_HIGH); // DigitalOut<B7,OUTPUT_AF_OPEN_DRAIN> sda;
        DigitalIO::SetAlternateFunction(B10,LL_GPIO_AF_4);
        DigitalIO::SetAlternateFunction(B11,LL_GPIO_AF_4);
      }
      ...
      if(m_I2Cx == nullptr)
      {
        ...
        DigitalIO::SetOutputMode(B8,OUTPUT_AF_OPEN_DRAIN,LL_GPIO_SPEED_FREQ_HIGH); // igitalOut<B6,OUTPUT_AF_OPEN_DRAIN> scl;
        DigitalIO::SetOutputMode(B9,OUTPUT_AF_OPEN_DRAIN,LL_GPIO_SPEED_FREQ_HIGH); // DigitalOut<B7,OUTPUT_AF_OPEN_DRAIN> sda;
        DigitalIO::SetAlternateFunction(B8,LL_GPIO_AF_4);
        DigitalIO::SetAlternateFunction(B9,LL_GPIO_AF_4);
      }
...
#if I2C_MASTER_DMA     
        InterruptManagerInstance.RegisterDeviceInterrupt(DMA1_Channel7_IRQn,0,&m_I2C_DMA_Rx_Callback);
        InterruptManagerInstance.RegisterDeviceInterrupt(DMA1_Channel6_IRQn,0,&m_I2C_DMA_Tx_Callback);
...
#if I2C_MASTER_DMA     
        InterruptManagerInstance.RegisterDeviceInterrupt(DMA1_Channel5_IRQn,0,&m_I2C_DMA_Rx_Callback);
        InterruptManagerInstance.RegisterDeviceInterrupt(DMA1_Channel4_IRQn,0,&m_I2C_DMA_Tx_Callback);
        InterruptManagerInstance.EnableInterrupt(DMA1_Channel5_IRQn);
        InterruptManagerInstance.EnableInterrupt(DMA1_Channel4_IRQn);
#endif 
      }   
      else
      {
...
#if I2C_MASTER_DMA || I2C_SLAVE_DMA
        dma1.HwInit();        
        dma1.ChanelHwInit(m_Curent_DMA_Tx_Channel,
                              (uint32_t)&(I2C_DATA_REG(m_I2Cx)),
                              ...
                              DMA_CCR_TEIE |  
                              LL_DMA_PRIORITY_LOW );         
        dma1.ChanelHwInit(m_Curent_DMA_Rx_Channel,
                              (uint32_t)&(I2C_DATA_REG(m_I2Cx)),
...
/************************I2C polling APIs END ************************/


/************************I2C Interrupt APIs Start ************************/

/************************I2C Interrupt APIs End ************************/
    ...
    I2c::I2CStatus_t I2c::SlaveStartListening(i2cBuf_t* TxBuf, i2cBuf_t* RxBuf )
    {
      ...
      m_CurrentIsr = &I2c::ISR_Slave_Intr;
    ...
    I2c::I2CStatus_t I2c::SlaveStartListening_DMA(i2cBuf_t* TxBuf, i2cBuf_t* RxBuf )
    {
      ...
      m_CurrentIsr = &I2c::ISR_Slave_DMA;
...
//        }
    ...
    I2c::I2CStatus_t I2c::XferIntr(Transaction_t const *pTransaction)  // 178 bytes
    {
      ...
      m_CurrentIsr = &I2c::ISR_Master_Intr; 
...
#pragma inline = forced
    ...
    {     
      ...
      m_CurrentIsr = &I2c::ISR_Master_DMA; 
...
#pragma inline = forced
    ...
    void I2c::I2C_DMA_Tx_Done_Handler()
    {	
#if 0
    ...
    void I2c::I2C_DMA_Rx_Done_Handler()
    {	
#if 0
      /* Transfer Complete Interrupt management ***********************************/
    ...
    void I2c::I2C_DMA_Rx_Callback::ISR( )
    {
      _This->I2C_DMA_Rx_Done_Handler();
    }
    
    void I2c::I2C_DMA_Tx_Callback::ISR( )
    {
      _This->I2C_DMA_Tx_Done_Handler();            
    }
    
#endif //  (I2C_MASTER_DMA == 1) || (I2C_SLAVE_DMA == 1)
    ...
    {  
      ...
      case EVENT_STOPF : 
        ...
        //m_SlaveTxn.RxBuf->Idx = m_SlaveTxn.RxBuf->Len - dma1.GetDataLen(I2C1_RX_DMA_CHANNEL);
         m_SlaveTxn.RxBuf->Idx = m_SlaveTxn.RxBuf->Len - LL_DMA_GetDataLength(DMA1,I2C1_RX_DMA_CHANNEL);
      ...
      case EVENT_AF : // NACK        
        ...
        //m_SlaveTxn.TxBuf->Idx = m_SlaveTxn.TxBuf->Len - dma1.GetDataLen(I2C1_TX_DMA_CHANNEL);  //LL_DMA_GetDataLength(m_DMAx,Channel);
        m_SlaveTxn.TxBuf->Idx = m_SlaveTxn.TxBuf->Len - LL_DMA_GetDataLength(DMA1,I2C1_TX_DMA_CHANNEL); 

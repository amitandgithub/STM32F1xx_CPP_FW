...
//extern I2C_HandleTypeDef hSSD1306_I2C;
/* Write command */
...
uint8_t SSD1306_Buffer1[(SSD1306_WIDTH * SSD1306_HEIGHT / 8)+1] = {0x40,};

uint8_t* SSD1306_Buffer = &SSD1306_Buffer1[1];

#define SSD1306_BUFFER_SIZE sizeof(SSD1306_Buffer1)-1
...
uint16_t counter;
  ...
  for(counter=0;counter<1024/8;counter++)
  {
    ons = ons%8;

     
     for(uint8_t i=0;i<8 - ons;i++)
     {
       SSD1306_WRITEDATA(0x00);
     }
     
    for(uint8_t i=0;i<ons;i++)
     {
...
extern I2CCallback_t I2CCallback;
  ...
  const HAL::I2c::Transaction_t Transaction =
  {
    ...
    .TxBuf              = SSD1306_Buffer1,
    .TxLen              = SSD1306_BUFFER_SIZE,
    .RxBuf              = 0,
    .RxLen              = 0,
    .XferDoneCallback   = &I2CCallback,
    .RepeatedStart      = 0
  };
volatile uint32_t i2c_dma_delay;
void SSD1306_DMA_Display(void) 
{ 
  uint32_t Xfercount = I2CCallback.get_XferComplete();
  ...
  i2c_dma_delay = SystickTimerTicks;
  SSD1306_I2C.XferDMA(&Transaction);
  //SSD1306_I2C.XferPoll(&Transaction);
  
  while(Xfercount == I2CCallback.get_XferComplete());

  i2c_dma_delay = SystickTimerTicks - i2c_dma_delay ; // 14ms @400K, 49ms @ 100K
}
...
//    dt[i+1] = data[i];
//
//  SSD1306_I2C.XferPoll(address,dt,count+1); 
//  //HAL_I2C_Master_Transmit(&hSSD1306_I2C, address, dt, count+1, 10);
//}
//
//
void ssd1306_I2C_Write(uint8_t address, uint8_t reg, uint8_t data) {
  ...
  dt[0] = reg;
  dt[1] = data;
  SSD1306_I2C.XferPoll(address,dt,2); 
  //HAL_I2C_Master_Transmit(&hSSD1306_I2C, address, dt, 2, 10);
}

...
class w25qxxDMACallback: public Callback
{
  public:
  virtual void CallbackFunction()
  {
...
void w25qxx_Test()
{
  ...
  if(InitDone == false)
  {
    ...
    w25qxxDev.HwInit();
    A3Pin.HwInit();
    for(uint16_t i = 0; i<sizeof(TxBuf); i++) TxBuf[i] = 0xB9;
  ...
  while(1)
  {
    ...
    case w25qxx_POLL_TXN:      
      ...
      Transaction.XferDoneCallback = &w25qxxDev.w25qxxCb;     
  ...
  case w25qxx_INTR_TX:  
      ...
      pBuffer[2] = (Page_Address& 0xFF00) >> 8;
      pBuffer[3] = Page_Address & 0xFF;    
      w25qxxDev.m_CurrentCallback = &w25qxxCb;  
      SPI.TxIntr(pBuffer,len+4,&w25qxxDev.w25qxxCb);
      while(!XferDone);  
      XferDone = false;     
      while( w25qxxDev.PageRead(Page,RxBuf) == BSP::w25qxx<FLASH_PARAMS>::W25QXX_BUSY);    
    ...
    case w25qxx_INTR_RX:    
      ...
      pBuffer[4] = 0x00;      
      SPI.TxPoll(pBuffer,5,100);
      SPI.RxIntr(RxBuf,len,&w25qxxDev.w25qxxCb);
      while(!XferDone);  
      XferDone = false; 
  ...
  case w25qxx_INTR_TXN_Q:      
      ...
      Transaction.RxLen = len;
      Transaction.XferDoneCallback = &w25qxxDev.w25qxxCb;     
      w25qxxDev.m_CurrentCallback = &w25qxxDMACb;
      while(SPI.Post(&Transaction,0) != Spi::SPI_TXN_QUEUE_ERROR) posts++;
  ...
  case w25qxx_DMA_TX:  
      ...
      pBuffer[3] = Page_Address & 0xFF;    
      w25qxxDev.m_CurrentCallback = &w25qxxCb;  
      SPI.TxDMA(pBuffer,len+4,&w25qxxDev.w25qxxCb);
      while(!XferDone);  
      XferDone = false;     
      while( w25qxxDev.PageRead(Page,RxBuf) == BSP::w25qxx<FLASH_PARAMS>::W25QXX_BUSY);    
    ...
    case w25qxx_DMA_RX:      
      ...
      pBuffer[4] = 0x00;      
      SPI.TxPoll(pBuffer,5,100);
      SPI.RxDMA(RxBuf,len,&w25qxxDev.w25qxxCb);
      while(!XferDone);  
      XferDone = false; 
   ...
   case w25qxx_DMA_TXN_Q:      
      ...
      Transaction.XferDoneCallback = &w25qxxDev.w25qxxCb;     
      w25qxxDev.m_CurrentCallback = &w25qxxDMACb;
      while(SPI.Post(&Transaction,1) != Spi::SPI_TXN_QUEUE_ERROR) posts++;
      while(DMAXferDone < posts);  
      test_id = 255;      
      break;
    case w25qxx_INTR:

...
namespace BSP
{
  ...
  ST7735::ST7735(ST7735_t ST7735t, HAL::Spi* pSpiDriverLCD, Port_t CSPort, PIN_t CSPin,Port_t DCPort, PIN_t DCPin, Port_t RstPort, PIN_t RstPin, Port_t BklPort, PIN_t BklPin): m_ST7735(ST7735t),m_Spi(pSpiDriverLCD),m_CSPin(CSPort,CSPin),m_DCPin(DCPort,DCPin),m_RstPin(RstPort,RstPin),m_BklPin(BklPort,BklPin)
  {
   ...
   void ST7735::WriteData(uint8_t* buff, size_t buff_size) 
  {
   ...
   void ST7735::SetAddressWindow(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1) {
    // column address set
    WriteCommand(CASET);
    uint8_t data[] = { 0x00, static_cast<uint8_t>(x0 + m_XSTART), 0x00, static_cast<uint8_t>(x1 + m_XSTART) };

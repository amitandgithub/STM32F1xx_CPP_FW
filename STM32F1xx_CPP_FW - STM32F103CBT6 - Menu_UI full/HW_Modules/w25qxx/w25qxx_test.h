...
#if NOKIA_BOARD
#define SPI spi2
  BSP::w25qxx w25qxxDev(&SPI,B12);
  HAL::DigitalOut<C6> HoldPin;
#else
  #define SPI spi1
  BSP::w25qxx w25qxxDev(&SPI,A3);
#endif
...
public:
  virtual void CallbackFunction(HAL::Spi::SpiStatus_t SPIStatus)
  {
    ...
    {
      SPI.SrartListeningTXIntr8bit(TxBuf,SLAVE_TX_LEN);
    }
    else  if(SPIStatus == Spi::SPI_SLAVE_TX_DONE )
    {
      SPI.SrartListeningRXIntr8bit(RxBuf,SLAVE_RX_LEN);
    }
#endif
#if SPI_SLAVE_DMA 
    if(SPIStatus == Spi::SPI_SLAVE_RX_DMA_DONE )
    {
      SPI.SrartListeningTXDma(TxBuf,SLAVE_TX_LEN);
    }  
    else  if(SPIStatus == Spi::SPI_SLAVE_TX_DMA_DONE )
    {
      SPI.SrartListeningRXDma(RxBuf,SLAVE_RX_LEN);
    }  
#endif
  }
};
...
{  
 ...
 // HAL::DigitalOut<A3> A3Pin;
  ...
  if(InitDone == false)
  {
    w25qxxDev.HwInit();
  //  A3Pin.HwInit();
    for(uint16_t i = 0; i<sizeof(TxBuf); i++) TxBuf[i] = 0xBE;

...
namespace HAL
{
  ...
  void Timer::BaseHwInit(LL_TIM_InitTypeDef* pTIM_InitStruct )
  {
    ...
    {
       LL_TIM_Init(m_TIMx, pTIM_InitStruct);
    }    
  }
  
  Timer::TimerStatus_t Timer::HwInit(LL_TIM_InitTypeDef* pTIM_InitStruct)
  {    
    if (m_TIMx == TIM2)
    {
  ...
  void Timer::PulseIn_HwInit(Chanel_t channel, uint8_t* Buf, uint16_t len, LL_TIM_InitTypeDef* pLL_TIM_InitStruct)
  {    
    LL_TIM_IC_InitTypeDef TIM_ICInitStruct;
    
    HwInit(pLL_TIM_InitStruct);
    ...
    EnableInterrupts( INTR_UIE | INTR_CC1IE | INTR_CC2IE);    //INTR_UIE
    m_TIMx->SR = 0;
    EnableTimerCounter();
  }
  ...
  void Timer::ISR()
  {
    ...
    if(LL_TIM_IsActiveFlag_CC1OVR(m_TIMx))
    {
      LL_TIM_ClearFlag_CC1OVR(m_TIMx);
    }
    
     if(LL_TIM_IsActiveFlag_CC2OVR(m_TIMx))
    {
      LL_TIM_ClearFlag_CC2OVR(m_TIMx);
    }

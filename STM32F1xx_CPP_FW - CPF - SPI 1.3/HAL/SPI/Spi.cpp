...
namespace HAL
{
  ...
  Spi::Spi(SpiPort_t SpiPort, Hz_t Hz) :
    ...
    void Spi::SetXferMode(SPI_Xfer_Mode_t SPI_Xfer_Mode)
    {
      
      m_Transaction.Xfer_Mode = SPI_Xfer_Mode;
      
      switch(SPI_Xfer_Mode)
      {
      case SPI_FULL_DUPLEX_TX_8_BIT: 
      case SPI_FULL_DUPLEX_RX_8_BIT:
      case SPI_FULL_DUPLEX_TXRX_8_BIT:
        SetTransmissionMode(LL_SPI_FULL_DUPLEX);
        LL_SPI_SetDataWidth(m_SPIx,LL_SPI_DATAWIDTH_8BIT);
        EnableInterrupt(SPI_CR2_RXNEIE | SPI_CR2_ERRIE );
        break;
        
      case SPI_FULL_DUPLEX_TX_16_BIT: 
      case SPI_FULL_DUPLEX_RX_16_BIT:
      case SPI_FULL_DUPLEX_TXRX_16_BIT:
        SetTransmissionMode(LL_SPI_FULL_DUPLEX);
        LL_SPI_SetDataWidth(m_SPIx,LL_SPI_DATAWIDTH_16BIT);
        EnableInterrupt(SPI_CR2_RXNEIE | SPI_CR2_ERRIE );
        break;
        
      case SPI_FULL_HALF_DUPLEX_TX_8_BIT: 
        SetTransmissionMode(LL_SPI_HALF_DUPLEX_TX);
        LL_SPI_SetDataWidth(m_SPIx,LL_SPI_DATAWIDTH_8BIT);
        EnableInterrupt(SPI_CR2_TXEIE | SPI_CR2_ERRIE );
        break;
        
      case SPI_FULL_HALF_DUPLEX_RX_8_BIT: 
        SetTransmissionMode(LL_SPI_HALF_DUPLEX_RX);
        LL_SPI_SetDataWidth(m_SPIx,LL_SPI_DATAWIDTH_8BIT);
        EnableInterrupt(SPI_CR2_RXNEIE | SPI_CR2_ERRIE );
        break;
        
      case SPI_FULL_HALF_DUPLEX_TX_16_BIT: 
        SetTransmissionMode(LL_SPI_HALF_DUPLEX_TX);
        LL_SPI_SetDataWidth(m_SPIx,LL_SPI_DATAWIDTH_16BIT);
        EnableInterrupt(SPI_CR2_TXEIE | SPI_CR2_ERRIE );
        break;
        
      case SPI_FULL_HALF_DUPLEX_RX_16_BIT: 
        SetTransmissionMode(LL_SPI_HALF_DUPLEX_RX);
        LL_SPI_SetDataWidth(m_SPIx,LL_SPI_DATAWIDTH_16BIT);
        EnableInterrupt(SPI_CR2_RXNEIE | SPI_CR2_ERRIE );
        break;
    ...
    Spi::SpiStatus_t Spi::TxIntr(uint8_t* TxBuf, uint32_t TxLen, SPI_Xfer_Mode_t SPI_Xfer_Mode, SPICallback_t XferDoneCallback)
    {      
      if(TxBuf == nullptr) return SPI_INVALID_PARAMS;
     ...
     // m_Transaction.Xfer_Mode = SPI_Xfer_Mode;
      ...
      if(SPI_Xfer_Mode != SPI_XFER_MODE_DEFAULT)
      {
        SetXferMode(SPI_Xfer_Mode);
      }
      else if(m_Transaction.Xfer_Mode == SPI_XFER_MODE_DEFAULT)
      {
        m_Transaction.Xfer_Mode = SPI_FULL_DUPLEX_TX_8_BIT;
      }
    ...
    Spi::SpiStatus_t Spi::RxIntr(uint8_t* RxBuf, uint32_t RxLen, SPI_Xfer_Mode_t SPI_Xfer_Mode, SPICallback_t XferDoneCallback)
    {      
      if(RxBuf == nullptr) return SPI_INVALID_PARAMS;
...
//      m_Transaction.Xfer_Mode = SPI_Xfer_Mode;
      ...
      {
        m_Transaction.Xfer_Mode = SPI_FULL_DUPLEX_RX_8_BIT;
      }
    ...
    {             
      ...
      switch(m_Transaction.Xfer_Mode)
      {
      case SPI_FULL_DUPLEX_TX_8_BIT: 

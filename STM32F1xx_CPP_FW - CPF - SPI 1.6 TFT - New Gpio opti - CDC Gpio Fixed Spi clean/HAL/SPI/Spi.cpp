...
namespace HAL
{
  ...
  Spi::Spi(SpiPort_t SpiPort, Hz_t Hz) :
    ...
    {     
      ...
      if(m_SPIState != SPI_RESET ) return SPI_OK;
      ...
      SPI_InitStruct.Mode              = LL_SPI_MODE_MASTER;//LL_SPI_MODE_MASTER;
...
//        GpioOutput sck(A5,OUTPUT_AF_PUSH_PULL);
//        GpioInput miso(A6,INPUT_PULLUP);
//        GpioOutput mosi(A7,OUTPUT_AF_PUSH_PULL);
        sck.HwInit();
        miso.HwInit();
        mosi.HwInit();
...
//        GpioOutput sck(B13,OUTPUT_AF_PUSH_PULL);
//        GpioInput miso(B14,INPUT_PULLUP);
//        GpioOutput mosi(B15,OUTPUT_AF_PUSH_PULL);
    ...
    {     
      SpiStatus_t SpiStatus = CheckAndLoadTxn(TxBuf,TxLen,TxBuf,0,mode,Spi_Baudrate,XferDoneCallback,CSPort,CSPinmask);
    ...
    {      
      SpiStatus_t SpiStatus = CheckAndLoadTxn(RxBuf,0,RxBuf,RxLen,mode,Spi_Baudrate,XferDoneCallback,CSPort,CSPinmask);
    ...
    {      
      ...
      if( SPI_RXNE(m_SPIx) )
      {
        ...
        else  if(m_SPIState == SPI_SLAVE_RX)
        {
          m_SlaveTxn.RxBuf->Buf[m_SlaveTxn.RxBuf->Idx++] = m_SPIx->DR;
          
          if(m_SlaveTxn.RxBuf->Idx >= m_SlaveTxn.RxBuf->Len)
          {
            m_SPIState = SPI_READY;
            DisableInterrupt(SPI_CR2_TXEIE | SPI_CR2_RXNEIE | SPI_CR2_ERRIE );            
            if(m_SlaveTxn.XferDoneCallback) m_SlaveTxn.XferDoneCallback->CallbackFunction(SPI_SLAVE_RX_DONE);             
            LL_SPI_ClearFlag_OVR(m_SPIx);
          }          
        }
        else  if(m_SPIState == SPI_SLAVE_TX)
        {
          //dummy = m_SPIx->DR; // to clear the Overflow flag
          LL_SPI_ClearFlag_OVR(m_SPIx);
          m_SPIx->DR = m_SlaveTxn.TxBuf->Buf[m_SlaveTxn.TxBuf->Idx++];
          
          if(m_SlaveTxn.TxBuf->Idx >= m_SlaveTxn.TxBuf->Len)
          {
             m_SPIState = SPI_READY;
             DisableInterrupt(SPI_CR2_TXEIE | SPI_CR2_RXNEIE | SPI_CR2_ERRIE );            
             if(m_SlaveTxn.XferDoneCallback) m_SlaveTxn.XferDoneCallback->CallbackFunction(SPI_SLAVE_TX_DONE); 
          }          
        }
        else  if(m_SPIState == SPI_SLAVE_TXRX)
        {
          m_SlaveTxn.RxBuf->Buf[m_SlaveTxn.RxBuf->Idx++] = m_SPIx->DR;
          m_SPIx->DR = m_SlaveTxn.TxBuf->Buf[m_SlaveTxn.TxBuf->Idx++];
          ...
          if(m_SlaveTxn.TxBuf->Idx >= m_SlaveTxn.TxBuf->Len)
          {
             m_SPIState = SPI_READY;
             DisableInterrupt(SPI_CR2_TXEIE | SPI_CR2_RXNEIE | SPI_CR2_ERRIE ); 
             if(m_SlaveTxn.XferDoneCallback) m_SlaveTxn.XferDoneCallback->CallbackFunction(SPI_SLAVE_TXRX_DONE);             
          }          
        }
      }// if( SPI_RXNE(m_SPIx) )  
      else
...
//      }
    ...
    void Spi::SrartListening(SPIState_t SPIState)
    {
      m_SPIState = SPIState;
      
      if(SPIState == SPI_SLAVE_TX)
      {
        EnableInterrupt(SPI_CR2_RXNEIE);
      }
      if(SPIState == SPI_SLAVE_RX)
      {
        EnableInterrupt(SPI_CR2_TXEIE);
      }
      //asm("bkpt 255");
    ...
    Spi::SpiStatus_t Spi::SlaveHwInit(SlaveTxn_t* SlaveTxn, uint32_t mode)
    {
      LL_SPI_InitTypeDef SPI_InitStruct;
      
      /* Set SPI_InitStruct fields to default values */
      SPI_InitStruct.TransferDirection = LL_SPI_FULL_DUPLEX;//LL_SPI_HALF_DUPLEX_TX;//LL_SPI_FULL_DUPLEX;
      SPI_InitStruct.Mode              = LL_SPI_MODE_SLAVE;//LL_SPI_MODE_MASTER;
      SPI_InitStruct.DataWidth         = LL_SPI_DATAWIDTH_8BIT;
      SPI_InitStruct.ClockPolarity     = LL_SPI_POLARITY_LOW;
      SPI_InitStruct.ClockPhase        = LL_SPI_PHASE_1EDGE;
      SPI_InitStruct.NSS               = LL_SPI_NSS_HARD_INPUT;//LL_SPI_NSS_SOFT;//LL_SPI_NSS_HARD_OUTPUT;
      SPI_InitStruct.BaudRate          = LL_SPI_BAUDRATEPRESCALER_DIV2;
      SPI_InitStruct.BitOrder          = LL_SPI_MSB_FIRST;
      SPI_InitStruct.CRCCalculation    = LL_SPI_CRCCALCULATION_DISABLE;
      SPI_InitStruct.CRCPoly           = 10U;         
      ...
      HAL::DigitalIn<A4,INPUT_PULLUP> ChipSelectPin;
      
      ChipSelectPin.HwInit();
      
      if( (SlaveTxn == nullptr) || ((SlaveTxn->TxBuf == nullptr) && (SlaveTxn->RxBuf == nullptr)) ) return SPI_INVALID_PARAMS;
      
      memcpy(&m_SlaveTxn,SlaveTxn,sizeof(SlaveTxn_t));
      
      if(mode == 0)
      {        
        m_SPIState = SPI_SLAVE_RX; 
      }
      else
      {

...
#define SPI_LOG_STATES_SIZE 500
  ...
  public:
    ...
    SpiStatus_t     TxIntr(uint8_t* TxBuf, uint32_t TxLen, XferMode_t mode = SPI_8BIT, SPICallback_t XferDoneCallback = nullptr, Port_t CSPort = (Port_t)0, uint16_t CSPinmask = 0);
    
    SpiStatus_t     RxIntr(uint8_t* RxBuf, uint32_t RxLen,  XferMode_t mode = SPI_8BIT, SPICallback_t XferDoneCallback = nullptr, Port_t CSPort = (Port_t)0, uint16_t CSPinmask = 0);
    ...
    SpiStatus_t     RxDMA(uint8_t* RxBuf, uint32_t RxLen, XferMode_t mode = SPI_8BIT, SPICallback_t XferDoneCallback = nullptr, Port_t CSPort = (Port_t)0, uint16_t CSPinmask = 0);
    
    SpiStatus_t     TxDMA(uint8_t* TxBuf, uint32_t TxLen, XferMode_t mode = SPI_8BIT, SPICallback_t XferDoneCallback = nullptr, Port_t CSPort = (Port_t)0, uint16_t CSPinmask = 0);

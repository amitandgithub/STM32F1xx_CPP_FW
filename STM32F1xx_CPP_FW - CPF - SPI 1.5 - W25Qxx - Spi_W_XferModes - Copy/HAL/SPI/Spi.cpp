...
namespace HAL
{
  ...
  Spi::Spi(SpiPort_t SpiPort, Hz_t Hz) :
    ...
    Spi::SpiStatus_t Spi::TxIntr(uint8_t* TxBuf, uint32_t TxLen, XferMode_t mode, SPICallback_t XferDoneCallback,Port_t CSPort, uint16_t CSPinmask)
    { 
    ...
    Spi::SpiStatus_t Spi::RxIntr(uint8_t* RxBuf, uint32_t RxLen, XferMode_t mode, SPICallback_t XferDoneCallback,Port_t CSPort, uint16_t CSPinmask)
    {      
      if(m_SPIState != SPI_READY) return SPI_BUSY;
    ...
    void Spi::LoadTxDMA()
    {
      if(m_Transaction.Mode == SPI_8BIT)
      {
        dma1.Load(m_Curent_DMA_Tx_Channel, (uint32_t)&(m_SPIx->DR),(uint32_t)m_Transaction.TxBuf,m_Transaction.TxLen, LL_DMA_DIRECTION_MEMORY_TO_PERIPH, LL_DMA_PDATAALIGN_BYTE);
      }
      else
      {
         dma1.Load(m_Curent_DMA_Tx_Channel, (uint32_t)&(m_SPIx->DR),(uint32_t)m_Transaction.TxBuf,m_Transaction.TxLen, LL_DMA_DIRECTION_MEMORY_TO_PERIPH, LL_DMA_PDATAALIGN_HALFWORD);
      }      
    }
    ...
    void Spi::LoadRxDMA()
    {
      if(m_Transaction.Mode == SPI_8BIT)
      {
        dma1.Load(m_Curent_DMA_Tx_Channel, (uint32_t)&(m_SPIx->DR),(uint32_t)m_Transaction.RxBuf,m_Transaction.RxLen, LL_DMA_DIRECTION_MEMORY_TO_PERIPH, LL_DMA_PDATAALIGN_BYTE);
        dma1.Load(m_Curent_DMA_Rx_Channel, (uint32_t)&(m_SPIx->DR),(uint32_t)m_Transaction.RxBuf,m_Transaction.RxLen, LL_DMA_DIRECTION_PERIPH_TO_MEMORY, LL_DMA_PDATAALIGN_BYTE);
      }
      else
      {
         dma1.Load(m_Curent_DMA_Tx_Channel, (uint32_t)&(m_SPIx->DR),(uint32_t)m_Transaction.RxBuf,m_Transaction.RxLen, LL_DMA_DIRECTION_MEMORY_TO_PERIPH, LL_DMA_PDATAALIGN_HALFWORD);
         dma1.Load(m_Curent_DMA_Rx_Channel, (uint32_t)&(m_SPIx->DR),(uint32_t)m_Transaction.RxBuf,m_Transaction.RxLen, LL_DMA_DIRECTION_PERIPH_TO_MEMORY, LL_DMA_PDATAALIGN_HALFWORD);
      }      
    }
    
    Spi::SpiStatus_t Spi::TxDMA(uint8_t* TxBuf, uint32_t TxLen, XferMode_t mode, SPICallback_t XferDoneCallback,Port_t CSPort, uint16_t CSPinmask)
    {      
      if(m_SPIState != SPI_READY) return SPI_BUSY;
    ...
    Spi::SpiStatus_t Spi::RxDMA(uint8_t* RxBuf, uint32_t RxLen, XferMode_t mode, SPICallback_t XferDoneCallback,Port_t CSPort, uint16_t CSPinmask)
    {      
      if(m_SPIState != SPI_READY) return SPI_BUSY;
    ...
    {      
      ...
      {  
        if(m_Transaction.Mode == SPI_8BIT)
        {
          dma1.Load(m_Curent_DMA_Rx_Channel, (uint32_t)&(m_SPIx->DR),(uint32_t)pTransaction->RxBuf,pTransaction->RxLen, LL_DMA_DIRECTION_PERIPH_TO_MEMORY, LL_DMA_PDATAALIGN_BYTE);
        }
        else
        {
          dma1.Load(m_Curent_DMA_Rx_Channel, (uint32_t)&(m_SPIx->DR),(uint32_t)pTransaction->RxBuf,pTransaction->RxLen, LL_DMA_DIRECTION_PERIPH_TO_MEMORY, LL_DMA_PDATAALIGN_HALFWORD);
        }

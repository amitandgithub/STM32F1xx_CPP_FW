...
{  
  ...
  case w25qxx_INTR_TX:  
      ...
      pBuffer[3] = Page_Address & 0xFF;   
      pBuffer[4] = pBuffer[5];
      SPI.TxIntr(pBuffer,len+4,_8_BIT,&w25qxxCb,Transaction.CSPort,Transaction.CSPinmask);
      while(!XferDone);  
      XferDone = false;     
      while( w25qxxDev.PageRead(Page,RxBuf) == BSP::w25qxx<FLASH_PARAMS>::W25QXX_BUSY);    
    ...
    case w25qxx_INTR_RX:    
      ...
      pBuffer[4] = 0x00;      
      SPI.TxPoll(pBuffer,5);
      SPI.RxIntr(RxBuf,len,_8_BIT,&w25qxxCb,Transaction.CSPort,Transaction.CSPinmask);
      while(!XferDone);  
      XferDone = false; 
  ...
  case w25qxx_DMA_TX:  
      ...
      pBuffer[3] = Page_Address & 0xFF;  
      pBuffer[4] = pBuffer[5];
      SPI.TxDMA(pBuffer,len+4,_8_BIT,&w25qxxCb,Transaction.CSPort,Transaction.CSPinmask);
      while(!XferDone);  
      XferDone = false;     
      while( w25qxxDev.PageRead(Page,RxBuf) == BSP::w25qxx<FLASH_PARAMS>::W25QXX_BUSY);    
    ...
    case w25qxx_DMA_RX:      
      ...
      pBuffer[4] = 0x00;      
      SPI.TxPoll(pBuffer,5);
      SPI.RxDMA(RxBuf,len,_8_BIT,&w25qxxCb,Transaction.CSPort,Transaction.CSPinmask); 
      while(!XferDone);  
      XferDone = false; 

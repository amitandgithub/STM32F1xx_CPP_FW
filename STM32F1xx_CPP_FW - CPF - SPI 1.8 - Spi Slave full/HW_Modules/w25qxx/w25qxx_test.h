...
SpiBuf_t SPI_Slave_TX_Buf = {TxBuf,15+1,0};
SpiBuf_t SPI_Slave_RX_Buf = {RxBuf,14+1,0};
...
public:
  virtual void CallbackFunction(HAL::Spi::SpiStatus_t SPIStatus)
  {
    ...
    {
      SPI.SrartListening(Spi::SPI_SLAVE_TX);
      SPI_Slave_RX_Buf.Idx = 0;
      //memset(TxBuf,RxBuf[3]+1,sizeof(TxBuf));
    }
    else  if(SPIStatus == Spi::SPI_SLAVE_TX_DONE )
    {
      SPI.SrartListening(Spi::SPI_SLAVE_RX);
      SPI_Slave_TX_Buf.Idx = 0; 
      //memset(RxBuf,0,20);
    }
    
//    if(SPIStatus == Spi::SPI_SLAVE_TX_DONE )
//    {
//      SPI.SrartListening(Spi::SPI_SLAVE_TX);
//      SPI_Slave_TX_Buf.Idx = 0; 
//      memset(TxBuf,index__++,sizeof(TxBuf));
//    }
//    else 
//      while(1);

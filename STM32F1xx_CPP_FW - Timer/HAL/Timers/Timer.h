...
{ 
  namespace Timer
  {
    ...
    template<uint32_t Timerx, uint32_t Prescaler, uint32_t Autoreload, bool CountUp = true>
      void HwInit(LL_TIM_InitTypeDef* pTIM_InitStruct = nullptr)
      {
    ...
    template<uint32_t Timerx, uint32_t TickFreq = uSecTimer>
      TimerStatus_t HwInit(LL_TIM_InitTypeDef* pTIM_InitStruct = nullptr)
      {
        ...
        {      
          TIM_InitStruct.Prescaler         = __LL_TIM_CALC_PSC (72000000, TickFreq);

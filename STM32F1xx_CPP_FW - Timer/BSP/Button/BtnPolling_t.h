...
**
******************/
#ifndef BtnPolling_t_h
#define BtnPolling_t_h
...
namespace BSP
{
 ...
 template<Port_t Port, PIN_t Pin,Mode_t Mode = INPUT_PULLUP, uint8_t Debounce = 0>
    ...
    public:
      ...
      BtnPolling_t() : m_previousState(BTN_IDLE),m_currentState(BTN_IDLE),m_startMillis(0)
      {
      }
      ...
      uint32_t getMillis(){ return HAL::GetTick();}
  ...
  template<Port_t Port, PIN_t Pin,Mode_t Mode, uint8_t Debounce>
    void BtnPolling_t<Port,Pin,Mode,Debounce>::HwInit()
    {
      m_BtnPin.HwInit();
    }    
  
  template<Port_t Port, PIN_t Pin,Mode_t Mode, uint8_t Debounce>
    BtnPolling_t<Port,Pin,Mode,Debounce>::BtnState_t
    BtnPolling_t<Port,Pin,Mode,Debounce>::Run()
    {

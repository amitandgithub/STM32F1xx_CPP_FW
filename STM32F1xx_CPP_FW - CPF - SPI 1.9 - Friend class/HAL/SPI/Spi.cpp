...
namespace HAL
{
  ...
  Spi::Spi(SpiPort_t SpiPort, Hz_t Hz) :
    ...
    void SPI1_DMA_Tx_Callback::ISR( )
    {      
#if SPI_MASTER_DMA && SPI_SLAVE_DMA 
      if((_this->m_SPIState == Spi::SPI_MASTER_TX_DMA) || (_this->m_SPIState == Spi::SPI_MASTER_RX_DMA) )
      {
        //_this->SPI1_DMA_MasterTx_Done_Handler();  
        //Spi::SPI1_DMA_MasterTx_Done_Handler();
        _this->m_CurrentIsr = nullptr;
      }
      else
      {

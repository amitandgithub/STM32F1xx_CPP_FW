...
namespace BSP
{
  template<Port_t CwPort, PIN_t CwPin, Port_t CCwPort, PIN_t CCwPin, Port_t SwPort, PIN_t SwPin, Callback_t OnCw = nullptr, Callback_t OnCCw = nullptr, Callback_t SwOnRise = nullptr, Callback_t SwOnFall = nullptr>
    ...
    private:
      BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>         m_Cw;
      BtnPolling<CCwPort,CCwPin,INPUT_PULLUP,0>       m_CCw;
      BtnPolling<SwPort,SwPin,INPUT_PULLUP,0>         m_Sw;        
    };
    ...
    Run()
    {
      typename BSP::BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BtnState_t CwState   =  m_Cw.Run();
      typename BSP::BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BtnState_t CCwState  =  m_CCw.Run();
      typename BSP::BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BtnState_t SwState   =  m_Sw.Run(); 
      
      if( (CwState == BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BTN_FALL) && (CCwState == BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BTN_RISE)  )
      {
        if(OnCw) OnCw->CallbackFunction(); 
      }
      else if( (CwState == BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BTN_RISE) && (CCwState == BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BTN_FALL)  )
      {
        if(OnCCw) OnCCw->CallbackFunction(); 
      }
      
      if(SwState == BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BTN_FALL)
      {
         if(SwOnFall) SwOnFall->CallbackFunction();
      }
      else if(SwState == BtnPolling<CwPort,CwPin,INPUT_PULLUP,0>::BTN_RISE)
      {

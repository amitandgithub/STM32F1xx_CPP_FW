...
static volatile uint32_t t1;
  ...
  void PowerMonitorInit()
  {
    ...
    TimerCount = rtc.ReadBackupReg(TimerCount_Word_2);
    Power.mAH  = rtc.ReadBackupReg(Power_mAH_Word_3);
...
{ 
  ...
  { 
    if(PrevTimerCount != rtc.ReadTimeCounter())
    {
      ...
      rtc.WriteBackupReg(TimerCount_Word_2,TimerCount);
      rtc.WriteBackupReg(Power_mAH_Word_3,(uint32_t)Power.mAH);
    }
  }

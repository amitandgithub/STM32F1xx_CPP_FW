...
void w25qxx_Test()
{
  ...
  if(InitDone == false)
  {
    ...
    w25qxxDev.HwInit();
    A3Pin.HwInit();
    for(uint16_t i = 0; i<sizeof(TxBuf); i++) TxBuf[i] = 0x2D;
  ...
  while(1)
  {
    ...
    case w25qxx_DMA:
      ...
      w25qxxDev.EraseSector(0);
      A3Pin.Toggle(); 
      while( w25qxxDev.PageWriteDMA(4,TxBuf,&w25qxxCb) == BSP::w25qxx<&spi1,A4>::W25QXX_BUSY); 
      while(!XferDone);
      A3Pin.Toggle();
      while( w25qxxDev.PageReadDMA(4,RxBuf,&w25qxxCb) == BSP::w25qxx<&spi1,A4>::W25QXX_BUSY);
      while(!XferDone);
      A3Pin.Toggle();
      test_id = 255;
      break;

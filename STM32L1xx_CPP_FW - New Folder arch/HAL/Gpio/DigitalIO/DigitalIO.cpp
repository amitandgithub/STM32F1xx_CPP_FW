...
{   
  ...
  void DigitalIO::SetInputMode(Port_t port, PIN_t pin, Mode_t mode, HAL::InterruptSource* This, IntEdge_t Edge, Callback_t Callback)
  {
    ...
    case INPUT_FLOATING:
      ...
      LL_GPIO_SetPinPull((GPIO_TypeDef*)port,LLpin,LL_GPIO_PULL_NO);
      break;
    case ANALOG:
  ...
  void DigitalIO::SetAlternateFunction(Port_t port, PIN_t pin,uint32_t AltFun)
  {     
    uint32_t LLpin = get_LL_pin(pin);
    
    if(LLpin > LL_GPIO_PIN_7)
    {
      LL_GPIO_SetAFPin_8_15((GPIO_TypeDef*)port,LLpin,AltFun);
    }
    else
    {
      LL_GPIO_SetAFPin_0_7((GPIO_TypeDef*)port,LLpin,AltFun);
    }

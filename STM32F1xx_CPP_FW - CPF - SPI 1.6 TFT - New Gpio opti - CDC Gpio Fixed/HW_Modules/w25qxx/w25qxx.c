...
static HAL::DigitalOut<A4> m_CSPin;
extern HAL::Spi spi1;
#define  SPI_X      spi1
#define  SPI_INIT   spi1.HwInit
#define  SPI_TXRX   spi1.TxRxPoll
#define  SPI_RX     spi1.RxPoll
#define  SPI_TX     spi1.TxPoll
...
#elif W25_INTR
  SPI_TX_INTR(pBuffer, NumByteToWrite_up_to_PageSize);
  while(SPI_X.GetState() != HAL::Spi::SPI_READY);
#else
  // DMA
#endif
...
bool writePageIntr(uint32_t addr, uint8_t *data) 
{
	W25qxx_WaitForWriteEnd();
    W25qxx_WriteEnable();
    m_CSPin.Low();
	W25qxx_Spi(0x02);
	(void)W25qxx_Spi(addr >> 16);
	(void)W25qxx_Spi(addr >>  8);
	(void)W25qxx_Spi(0); // If len=256, page boundary only (datasheet 7.2.21)
//	for(int i=0; i<256; i++) 
//    {
//		(void)W25qxx_Spi(data[i]);
//	}
    
    SPI_TX_INTR(data, 256);
    
    while(SPI_X.GetState() != HAL::Spi::SPI_READY);
    
	m_CSPin.High(); // Write occurs after the CS line is de-asserted

	W25qxx_Delay(3);     // Max page program time according to datasheet
...
void W25qxx_ReadBytes(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
  ...
  W25qxx_Spi(0);
  SPI_RX(pBuffer,NumByteToRead,200);	
  m_CSPin.High();
#if (_W25QXX_DEBUG==1)
  StartTime = HAL::GetTick()-StartTime; 
...
#elif W25_INTR
  SPI_RX_INTR(pBuffer, NumByteToRead_up_to_PageSize);
  while(SPI_X.GetState() != HAL::Spi::SPI_READY);
#else
  // DMA
#endif

  ...
  public:   
    ...
    void CopyName(uint8_t NameIndex)
    {
      ...
      while( (idx < m_SettingContext->Settinglen - m_SettingContext->SettingVariablePosition) ) // (m_SettingContext->pNamedSettingTable[NameIndex].SettingText[idx] != 0) && 
      {
        ...
        {
          m_SettingText[m_SettingContext->SettingVariablePosition + idx] = m_SettingContext->pNamedSettingTable[NameIndex].SettingText[idx];
        }
        else
        {
          m_SettingText[m_SettingContext->SettingVariablePosition + idx] = ' '; // Add space if the string is shorter than the total length
        }
          idx++;
      }
    }

...
namespace HAL
{
  ...
  Spi::Spi(SpiPort_t SpiPort, Hz_t Hz) :
    ...
    {      
      ...
      if( SPI_RXNE(m_SPIx) )
      {
        ...
        else  if(m_SPIState == SPI_SLAVE_RX)
        {
          ...
          {
             if(m_SlaveTxn.XferDoneCallback) m_SlaveTxn.XferDoneCallback->CallbackFunction(SPI_OK);
             //m_SPIState = SPI_READY;
          }          
        }
      }// if( SPI_RXNE(m_SPIx) )  
    ...
    Spi::SpiStatus_t Spi::StartListening(SlaveTxn_t* SlaveTxn, uint32_t mode)
    {
      LL_SPI_InitTypeDef SPI_InitStruct;
      ...
      HAL::DigitalOut<A4,OUTPUT_AF_OPEN_DRAIN> ChipSelectPin;

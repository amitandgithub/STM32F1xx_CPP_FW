...
public:
  virtual void CallbackFunction(HAL::Spi::SpiStatus_t SPIStatus)
  {
    ...
    {
      //SPI.SrartListening(Spi::SPI_SLAVE_TX,TxBuf,14);
      SPI.SrartListeningTXIntr(TxBuf,14);
    }
    else  if(SPIStatus == Spi::SPI_SLAVE_TX_DONE )
    {
      //SPI.SrartListening(Spi::SPI_SLAVE_RX,RxBuf,15);
      SPI.SrartListeningRXIntr(RxBuf,15);
    }    
  }
};
...
{  
  ...
  test_id = POLL_TX_SlaveBrd;//POLL_TX_SlaveBrd;
  while(1)
  {
  ...
  case w25qxx_DMA_TX:  
   ...
   case w25qxx_DMA_TXN_Q: 
    ...
    case POLL_TX_SlaveBrd:
      ...
      Transaction.TxLen = 15;
      Transaction.RxBuf = RxBuf;
      Transaction.RxLen = 14;
      Transaction.XferDoneCallback = &SpiSlaveCb;
      SPI.HwInit(true);
      //SPI.SrartListening(Spi::SPI_SLAVE_RX,RxBuf,14);
      SPI.SrartListeningRXIntr(RxBuf,15);
      while(1) Pc_Count++;
      break;

  ...
  Spi::Spi(SpiPort_t SpiPort, Hz_t Hz) :
    ...
    {     
      ...
      if(m_SPIx == SPI1)
      {
        if(Slave)
        {
          DigitalIn<A4,INPUT_PULLDOWN> ChipSelectPin;
          DigitalIn<A5,INPUT_PULLDOWN> sck;      
          DigitalOut<A6,OUTPUT_AF_PUSH_PULL> miso;
          DigitalIn<A7,INPUT_PULLDOWN> mosi;
        ...
        {
          // These templates generate less code than non-template class
      ...
      {        
        if(Slave)
        {
          ...
          DigitalIn<B12,INPUT_PULLDOWN> ChipSelectPin;          
    ...
    LL_SPI_Init(m_SPIx, &SPI_InitStruct);    
      ...
      //LL_SPI_SetDataWidth(m_SPIx, (mode == SPI_8BIT) ? LL_SPI_DATAWIDTH_8BIT : LL_SPI_DATAWIDTH_16BIT);
    ...
    template<Port_t ClkPort, PIN_t ClkPin,Port_t MisoPort, PIN_t MisoPin,Port_t MosiPort, PIN_t MosiPin, Port_t ChipSelectPort, PIN_t ChipSelectPin>
      void Spi::SlavePinsHwInit()
      {
        // These templates generate less code than none template class
...
//      HAL::DigitalIn<A4,INPUT_PULLDOWN> ChipSelectPin;
//      HAL::DigitalIn<A5,INPUT_PULLDOWN> Clock;      
//      HAL::DigitalOut<A6,OUTPUT_AF_PUSH_PULL> Miso;
//      HAL::DigitalIn<A7,INPUT_PULLDOWN> Mosi; 
    ...
    {      
      ...
      m_Direction = SPI_DIRECTION_TXRX;
    ...
    {      
      ...
      if( SPI_RXNE(m_SPIx) )
      {
        if(m_Direction == SPI_DIRECTION_TX)
        {
          ...
          {               
            ...
            else
            {
              ...
              {
                SpiStatus = SPI_SLAVE_TX_DONE;
              }
              
              if(m_Transaction.XferDoneCallback) m_Transaction.XferDoneCallback->CallbackFunction(SpiStatus);             
              LL_SPI_ClearFlag_OVR(m_SPIx);
            }
          }
        }
        else if(m_Direction == SPI_DIRECTION_RX)
        {
        ...
        else  if(m_Direction == SPI_DIRECTION_TXRX)
        {
          if(m_Transaction.TxLen > 0) 
          {            
            if(m_Transaction.Mode == SPI_8BIT)
          ...
          else
          {
            ...
            {
              DisableInterrupt(SPI_CR2_TXEIE | SPI_CR2_RXNEIE | SPI_CR2_ERRIE );            
              if(m_Transaction.XferDoneCallback) m_Transaction.XferDoneCallback->CallbackFunction(SPI_SLAVE_TXRX_DONE);             
              LL_SPI_ClearFlag_OVR(m_SPIx);
            }
          }
        }
    ...
    void Spi::SrartListeningTXIntr(uint8_t* Buf, uint16_t Len)
    {
      m_SPIState = SPI_SLAVE_TX;
    ...
    void Spi::SrartListeningRXIntr(uint8_t* Buf, uint16_t Len)
    {
    ...
    void Spi::SrartListening(SPIState_t SPIState, uint8_t* Buf, uint16_t Len)
    {
      m_SPIState = SPIState;
      ...
      else if(SPIState == SPI_SLAVE_RX_DMA)
      {
         LoadRxDmaChannel(Buf,Len);
      }
      else if(SPIState == SPI_SLAVE_TX_DMA)
      {
        LoadTxDmaChannel(Buf,Len);
      }      
    }
    
//    void Spi::SlaveHwInit(XferMode_t mode)
//    {
//      LL_SPI_InitTypeDef SPI_InitStruct;
//      
//      /* Set SPI_InitStruct fields to default values */

...
//    }Transaction_t; // 28 bytes, It generates more ram data and code
    ...
    void LoadRxDmaChannel(uint8_t* Buf, uint32_t len){dma1.Load(I2C1_RX_DMA_CHANNEL, (uint32_t)&(I2C_DATA_REG(m_I2Cx)),(uint32_t)Buf,len, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);}
    
    void LoadTxDmaChannel(uint8_t* Buf, uint32_t len){dma1.Load(I2C1_TX_DMA_CHANNEL, (uint32_t)&(I2C_DATA_REG(m_I2Cx)),(uint32_t)Buf,len, LL_DMA_DIRECTION_MEMORY_TO_PERIPH);}
#endif

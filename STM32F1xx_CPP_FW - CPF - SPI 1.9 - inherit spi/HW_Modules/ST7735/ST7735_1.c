...
HAL::GpioOutput m_RstPin(B0);
HAL::GpioOutput m_DCPin(B1);
HAL::GpioOutput m_CSPin(B2);

#define ST7735_CS       m_CSPin
#define ST7735_RESET    m_RstPin
#define ST7735_DC       m_DCPin
...
void ST7735_test(){
    ...
    for (x = 0; x <= 500; x += 50){
        ...
        ST7735_drawString(1, 1, (char*)"Reading", colMediumOrchid, 3);
        LL_mDelay(1000);
...
void ST7735_writeInteger(int x, int y, int v, unsigned int colour, int size)
{
    ...
    ST7735_drawString(x, y, (char*)buf, colour, size);

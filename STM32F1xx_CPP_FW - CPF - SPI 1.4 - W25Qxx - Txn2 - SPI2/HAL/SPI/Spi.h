...
#define SPI_LOG_STATES_SIZE 500
  ...
  public:
    ...
    SpiStatus_t     RxIntr(uint8_t* RxBuf, uint32_t RxLen, SPICallback_t XferDoneCallback = nullptr);
    ...
    SpiStatus_t     RxDMA(uint8_t* RxBuf, uint32_t RxLen, SPICallback_t XferDoneCallback = nullptr);
    
    SpiStatus_t     TxDMA(uint8_t* TxBuf, uint32_t TxLen, SPICallback_t XferDoneCallback = nullptr);
    ...
    void LoadRxDmaChannel(uint8_t* Buf, uint32_t len){dma1.Load(SPI1_RX_DMA_CHANNEL, (uint32_t)&(m_SPIx->DR),(uint32_t)Buf,len, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);}
    
    void LoadTxDmaChannel(uint8_t* Buf, uint32_t len){dma1.Load(SPI1_TX_DMA_CHANNEL, (uint32_t)&(m_SPIx->DR),(uint32_t)Buf,len, LL_DMA_DIRECTION_MEMORY_TO_PERIPH);}

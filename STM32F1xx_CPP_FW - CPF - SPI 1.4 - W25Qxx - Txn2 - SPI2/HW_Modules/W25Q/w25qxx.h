...
#define w25qxx_TEMPLATE_T pSpiDriver,CSPort,CSPin
  ...
  template<w25qxx_TEMPLATE_PARAMS>
    ...
    using Callback_t = HAL::Spi::SPICallback_t;
     ...
     // Status_t PageWriteBytes(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback = nullptr);
      
     // Status_t PageRead(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback = nullptr);
...
//        pSpiDriver->TxPoll(pBuffer,5,100);
//        pSpiDriver->RxPoll(pBuffer,len,100);
        m_CSPin.High();
      }      
      return W25QXX_SUCCESS;
    }  

...
namespace HAL
{
  ...
  Spi::Spi(SpiPort_t SpiPort, Hz_t Hz) :
    ...
    Spi::SpiStatus_t Spi::SlaveHwInit(SlaveTxn_t* SlaveTxn, uint32_t mode)
    {
      ...
      memcpy(&m_Transaction,SlaveTxn,sizeof(SlaveTxn_t));
...
#if SPI_SLAVE_DMA  
        LoadTxDmaChannel(m_Transaction.TxBuf->Buf,m_Transaction.TxBuf->Len);
        LoadRxDmaChannel(m_Transaction.RxBuf->Buf,m_Transaction.RxBuf->Len); 
#endif
      }      
      EnableInterrupt(SPI_CR2_RXNEIE);      
      return SPI_OK;

...
{    
  ...
  uint32_t MaxLogEntries = 1000U >                  
    ...
    public:
      ...
      static constexpr DebugLog_t TIMESTAMP_MASK = ( (~((DebugLog_t)0))>>( (sizeof(DebugLog_t)*8) - (LogValueBits+ModuleIDBits))) << TimeStampBits ;
    ...
    public:
      ...
      static void     Log                     (char const* Buf, uint16_t Len);
...
//     }
     ...
     {
       for(uint32_t modulecount = 0; modulecount < LOG_MODULE_ID_MAX; modulecount++)
       {
         if((m_RegisteredModuleIDs & modulecount) == 0)
         {
           m_RegisteredModuleIDs |= (DebugLog_t)1U << modulecount;
           *pModuleID = (DebugLog_t)1U << modulecount;
      ...
      template< typename T,uint32_t ModuleIDBits, uint32_t LogValueBits,uint32_t TimeStampBits,uint32_t MaxLogEntries > 
       void DebugLog< T,ModuleIDBits,LogValueBits,TimeStampBits,MaxLogEntries >::
       Log(char const* Buf, uint16_t Len)
       {
#if USB_DEVICE

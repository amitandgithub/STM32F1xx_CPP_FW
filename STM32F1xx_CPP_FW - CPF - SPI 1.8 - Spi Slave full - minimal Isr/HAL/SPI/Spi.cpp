...
//        GpioOutput mosi(B15,OUTPUT_AF_PUSH_PULL);
    ...
    Spi::SpiStatus_t Spi::SlaveHwInit(Transaction_t const* SlaveTxn, uint32_t mode)
    {
      LL_SPI_InitTypeDef SPI_InitStruct;
      ...
      memcpy(&m_Transaction,SlaveTxn,sizeof(Transaction_t));
...
#if SPI_SLAVE_DMA  
        LoadTxDmaChannel(SlaveTxn->TxBuf,SlaveTxn->TxLen);
        LoadRxDmaChannel(SlaveTxn->RxBuf,SlaveTxn->RxLen); 
#endif
      }      
      EnableInterrupt(SPI_CR2_RXNEIE);      
      return SPI_OK;

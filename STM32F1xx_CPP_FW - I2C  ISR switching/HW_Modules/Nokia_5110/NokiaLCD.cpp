...
namespace BSP
{ 
  extern const int8_t ASCII[][6];
  
  NokiaLCD::NokiaLCD(SPIDrv_t* pSpi,Port_t CSPort, PIN_t CSPin,Port_t DCPort, PIN_t DCPin, Port_t RstPort, PIN_t RstPin, Port_t BklPort, PIN_t BklPin): 
    m_pSpi(pSpi),
    m_CS(CSPort,CSPin),
    m_DC(DCPort,DCPin),
    ...
    bool NokiaLCD::DisplayInit()
    {
      //Reset the LCD to a known state
      
      m_Reset.Low();
      
      LL_mDelay(1); // 1ms delay for reset pin
      
      m_Reset.High();
      
      Write(COMMAND, 0x21); //Tell LCD that extended commands follow
      Write(COMMAND, 0xB1); //Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark, Amit: 0xBF works fine with 3.3v
    ...
    //the data byte
    void NokiaLCD::Write(DC_t DC, unsigned char data)
    {
      ...
      m_pSpi->TxPoll(&data,1);    
    }
    
    
    void NokiaLCD::SetMode(uint8_t LCDMode)
    {
      if(LCDMode == INVERSE)
      {
    ...
    void NokiaLCD::GoToXY(uint8_t x, uint8_t y)
    {
    ...
    //void NokiaLCD::DrawBitmap(const char my_array[])
    //{
    //	int index;
    //	for (index = 0 ; index < (LCD_X * LCD_Y / 8) ; index++)
    ...
    void NokiaLCD::LCDCharacter(const char character)
    {
      int32_t index;
    ...
    void NokiaLCD::DrawString(const char *characters)
    {
      while (*characters)
        LCDCharacter(*characters++);
    }
    
    void NokiaLCD::DisplayChar(unsigned char Row, unsigned char Col, const char aChar)
    {
      GoToXY(Col*SIZE_OF_1_CHAR,Row);
      LCDCharacter(aChar);
      
    }
    void NokiaLCD::DisplayStr(unsigned char Row, unsigned char Col, const char* Str)
    {
      uint8_t i=0;
      GoToXY(Col*SIZE_OF_1_CHAR,Row);
      DrawString("              ");
    ...
    void NokiaLCD::DisplayBuf(char* pBuffer)
    {
    ...
    NokiaLCD::Status_t NokiaLCD::Refresh( )
    {
      ...
      //        
      //        return m_pSpi->Post(&m_CurrentTransaction);
      //        
      //    }
      return 1;

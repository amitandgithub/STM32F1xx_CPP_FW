    ...
    template<TimerChanel_t Chanel>
      void PinsHwInit(bool Input)
      {
        ...
        if(Input == true)
        {
          ...
          HAL::DigitalIO::SetInputMode(port,pin,INPUT_FLOATING); //Rx.HwInit(INPUT_PULLDOWN);
        }
        else
        {
          //HAL::GpioOutput TimerPin(port,pin);
          //TimerPin.HwInit(OUTPUT_AF_PUSH_PULL);  // Output Alternate function Push-Pull
          HAL::DigitalIO::SetOutputMode(port,pin,OUTPUT_AF_PUSH_PULL,LL_GPIO_SPEED_FREQ_HIGH); // Tx.HwInit(OUTPUT_AF_PUSH_PULL);
        }
      }   

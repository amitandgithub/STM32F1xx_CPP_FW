...
#define w25qxx_TEMPLATE_T pSpiDriver,CSPort,CSPin
  ...
  template<w25qxx_TEMPLATE_PARAMS>
    ...
    //typedef HAL::Spi::SPICallback_t* Callback_t;
    ...
    using Callback_t = HAL::Spi::SPICallback_t;
      ...
      void WritePage(uint8_t *pBuffer, uint32_t Page_Address, uint32_t OffsetInByte = 0, uint32_t NumByteToWrite_up_to_PageSize = 256, HAL::Callback* Callback = nullptr );
      
      void ReadPage(uint8_t *pBuffer, uint32_t Page_Address, uint32_t OffsetInByte = 0, uint32_t NumByteToRead_up_to_PageSize = 256, HAL::Callback* Callback = nullptr );      
      
      bool HwInit();
      
      Status_t PageWrite(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback = nullptr, uint16_t len = 256);
      
      Status_t PageRead(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback = nullptr, uint16_t len = 256);
      
      Status_t PageWriteDMA(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback = nullptr, uint16_t len = 256);
      
      Status_t PageReadDMA(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback = nullptr, uint16_t len = 256);
  ...
  //###################################################################################################################
  template<w25qxx_TEMPLATE_PARAMS>
    void w25qxx<w25qxx_TEMPLATE_T>::WritePage(uint8_t *pBuffer	,uint32_t Page_Address,uint32_t OffsetInByte,uint32_t NumByteToWrite_up_to_PageSize,HAL::Callback* Callback)
    {
      while(m_DevInfo.Lock==1)
        W25qxx_Delay(1);
      m_DevInfo.Lock=1;
  ...
  //###################################################################################################################
  template<w25qxx_TEMPLATE_PARAMS>
    void w25qxx<w25qxx_TEMPLATE_T>::ReadPage(uint8_t *pBuffer,uint32_t Page_Address,uint32_t OffsetInByte,uint32_t NumByteToRead_up_to_PageSize,HAL::Callback* Callback)
    {
      while(m_DevInfo.Lock==1)
        W25qxx_Delay(1);
      m_DevInfo.Lock=1;
  ...
  template<w25qxx_TEMPLATE_PARAMS>
    ...
    Status_t w25qxx<w25qxx_TEMPLATE_T>::PageWrite(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback, uint16_t len )
    {
      ...
      if(Callback)
      {
        ...
        pSpiDriver->TxIntr(pBuffer,len+4,&w25qxxCb);
  ...
  template<w25qxx_TEMPLATE_PARAMS> 
    w25qxx<w25qxx_TEMPLATE_T>::
    Status_t w25qxx<w25qxx_TEMPLATE_T>::PageRead(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback, uint16_t len )
    {
      ...
      if(Callback)
      {
        ...
        pSpiDriver->XferIntr(&Transaction);
         ...
         //pSpiDriver->RxIntr(pBuffer,len,&w25qxxCb);        
      }
      else
      {
  ...
  template<w25qxx_TEMPLATE_PARAMS>
    w25qxx<w25qxx_TEMPLATE_T>::
    Status_t w25qxx<w25qxx_TEMPLATE_T>::PageWriteDMA(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback, uint16_t len )
    {
      if(pBuffer == nullptr) return W25QXX_ERROR;
  ...
  template<w25qxx_TEMPLATE_PARAMS> 
    w25qxx<w25qxx_TEMPLATE_T>::
  Status_t w25qxx<w25qxx_TEMPLATE_T>::PageReadDMA(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback, uint16_t len )
  {
    Transaction_t Transaction;
    if(pBuffer == nullptr) return W25QXX_ERROR;
    ...
    pSpiDriver->TxPoll(pBuffer,5,100);
    pSpiDriver->RxDMA(pBuffer,len,&w25qxxCb);

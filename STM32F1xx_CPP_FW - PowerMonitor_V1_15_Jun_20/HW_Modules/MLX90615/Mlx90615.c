...
#else
#define	Mlx90615_Delay(x)		LL_mDelay(x)
#endif
...
void	Mlx90615_ReadConfig(void)
{
    ...
    MLX_I2C.XferPoll(_MLX90615_I2C_ADDRESS,Add,1,Data,2,1);
	//I2C_Mem_Read(&_MLX90615_I2C,_MLX90615_I2C_ADDRESS,0x12,I2C_MEMADD_SIZE_8BIT,Data,2,100);
	memcpy(&Mlx90615,Data,2);
    
    Add[0] = 0x13;
    MLX_I2C.XferPoll(_MLX90615_I2C_ADDRESS,Add,1,Data,2,1);
	//I2C_Mem_Read(&_MLX90615_I2C,_MLX90615_I2C_ADDRESS
	//I2C_Mem_Read(&_MLX90615_I2C,_MLX90615_I2C_ADDRESS,0x13,I2C_MEMADD_SIZE_8BIT,Data,2,100);
	Mlx90615.Emissivity = (Data[0]|(Data[1]<<8))/16384.0f;
}
//##################################################################################################
bool	Mlx90615_Init(void)
{
  MLX_I2C.HwInit(400000);
  
	while(HAL::GetTick()<600)
		Mlx90615_Delay(10);
	//if(HAL_I2C_IsDeviceReady(&_MLX90615_I2C,_MLX90615_I2C_ADDRESS,1,20)!=HAL_OK)
		//return false;
    //while(MLX_I2C.Ping(_MLX90615_I2C_ADDRESS) != true);
	Mlx90615_ReadConfig();	
	return true;
}
uint8_t crc;
//################################################################################################
bool	Mlx90615_ReadTempTarget(float *Temp, uint8_t Far)
{	
  uint8_t	Add[2];
	uint8_t	Data[3];
	//if(I2C_Mem_Read(&_MLX90615_I2C,_MLX90615_I2C_ADDRESS,0x27,I2C_MEMADD_SIZE_8BIT,Data,3,100)!=HAL_OK)
		//return false;
    
     Add[0] = 0x27;
     MLX_I2C.XferPoll(_MLX90615_I2C_ADDRESS,Add,1,Data,3,1);
...
{	
    uint8_t	Add[2];
	uint8_t	Data[3];
	//if(I2C_Mem_Read(&_MLX90615_I2C,_MLX90615_I2C_ADDRESS,0x26,I2C_MEMADD_SIZE_8BIT,Data,3,100)!=HAL_OK)
		//return false;
...
void	Mlx90615_EnterSleep(void)
{
    ...
    MLX_I2C.XferPoll(_MLX90615_I2C_ADDRESS,Data,2);
...
bool	_Mlx90615_SetEmissivity(uint16_t	Emissivity)
{
	...
	*data = crc8Msb(0x07, (uint8_t*)buffer, 4);
	...
	//	return false;
     ...
     Add[0] = 0x13;
     MLX_I2C.XferPoll(_MLX90615_I2C_ADDRESS,Add,1); // Write mem Address
     MLX_I2C.XferPoll(_MLX90615_I2C_ADDRESS,&buffer[2],3); // Write mem Data
     
	Mlx90615_Delay(100);	
	buffer[0]=0;
	buffer[1]=0;
	//I2C_Mem_Read(&_MLX90615_I2C,_MLX90615_I2C_ADDRESS,0x13,I2C_MEMADD_SIZE_8BIT,buffer,2,100);
    
     Add[0] = 0x13;
     MLX_I2C.XferPoll(_MLX90615_I2C_ADDRESS,Add,1,buffer,2,1);
...
bool	Mlx90615_SetEmissivity(float	Emissivity)
{
	if(_Mlx90615_SetEmissivity(0)==true)
	{
		...
		if(Emissivity<0)
			Emissivity=0;
		return _Mlx90615_SetEmissivity((uint16_t)(Emissivity*16384.0));		
	}
	else
		return false;
}
...
bool	_Mlx90615_SetIIR(uint8_t	IIR_1_to_7)
{
	...
	*data = crc8Msb(0x07, (uint8_t*)buffer, 4);
	...
	buffer[0]=0;
	buffer[1]=0;
	//I2C_Mem_Read(&_MLX90615_I2C,_MLX90615_I2C_ADDRESS,0x12,I2C_MEMADD_SIZE_8BIT,buffer,2,100);
    Add[0] = 0x12;
    MLX_I2C.XferPoll(_MLX90615_I2C_ADDRESS,Add,1,buffer,2,1);

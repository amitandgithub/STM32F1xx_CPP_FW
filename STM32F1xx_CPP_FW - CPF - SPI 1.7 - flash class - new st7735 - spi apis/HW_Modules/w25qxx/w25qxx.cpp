...
namespace BSP
{
  ...
  w25qxx::w25qxx(HAL::Spi*   pSpiDriver, Port_t CSPort, PIN_t CSPin) : m_SpiDriver(pSpiDriver), m_CSPin(CSPort,CSPin)
  {
    
  }
  //###################################################################################################################
  
    uint8_t w25qxx::SpiTxRxByte(uint8_t Data)
    {
      uint8_t	data,ret;
      data = Data;
      //HAL_SPI_TransmitReceive(&_SpiTxRxByte,&Data,&ret,1);
      m_SpiDriver->TxRxPoll(&data,&ret,1);
      return ret;	
    }
  ...
  //###################################################################################################################
    ...
    void w25qxx::WritePage(uint8_t *pBuffer ,uint32_t Page_Address,uint32_t OffsetInByte,uint32_t NumByteToWrite_up_to_PageSize,HAL::Spi::spiCallback* Callback)
    {
      while(m_DevInfo.Lock==1)
        W25qxx_Delay(1);
      m_DevInfo.Lock=1;
      ...
      if(Callback)
      {
        m_SpiDriver->TxIntr(pBuffer, NumByteToWrite_up_to_PageSize);
        while(m_SpiDriver->GetState() != HAL::Spi::SPI_READY);
      }
      else
      {
        m_SpiDriver->TxPoll(pBuffer,NumByteToWrite_up_to_PageSize);
      }
  ...
  //###################################################################################################################
    ...
    void w25qxx::ReadPage(uint8_t *pBuffer,uint32_t Page_Address,uint32_t OffsetInByte,uint32_t NumByteToRead_up_to_PageSize,HAL::Spi::spiCallback* Callback)
    {
      while(m_DevInfo.Lock==1)
        W25qxx_Delay(1);
      m_DevInfo.Lock=1;
      ...
      if(Callback)
      {
        m_SpiDriver->RxIntr(pBuffer, NumByteToRead_up_to_PageSize);
        while(m_SpiDriver->GetState() != HAL::Spi::SPI_READY);
      }
      else
      {
        m_SpiDriver->RxPoll(pBuffer,NumByteToRead_up_to_PageSize);
      }
      m_CSPin.High();
#if (_W25QXX_DEBUG==1)
      StartTime = HAL::GetTick()-StartTime; 
   ...
   //###################################################################################################################
    w25qxx::Status_t w25qxx::PageWrite(uint32_t Page_Address, uint8_t* pBuffer, HAL::Spi::spiCallback* Callback, uint16_t len )
    {
      ...
      if(Callback)
      {
        m_SpiDriver->TxIntr(pBuffer,len+4,HAL::Spi::SPI_8BIT,HAL::Spi::SPI_BAUDRATE_DIV2,Callback);
        //m_SpiDriver->TxDMA(pBuffer,len+4,&w25qxxCb);
      }
      else
      {
        m_SpiDriver->TxPoll(pBuffer,len+4);
        m_CSPin.High();
      }      
  ...
  //###################################################################################################################
    ...
    w25qxx::Status_t w25qxx::PageRead(uint32_t Page_Address, uint8_t* pBuffer, HAL::Spi::spiCallback* Callback, uint16_t len )
    {
      ...
      Transaction.pChipselect = &m_CSPin;
      Transaction.XferDoneCallback = Callback;
      
      if(Callback)
      {       
        m_SpiDriver->XferIntr(&Transaction);       
      }
      else
      {
          m_SpiDriver->XferPoll(&Transaction);
          m_CSPin.High();
      }      
      return W25QXX_SUCCESS;
    }  
  ...
  w25qxx::Status_t w25qxx::PageWriteDMA(uint32_t Page_Address, uint8_t* pBuffer, HAL::Spi::spiCallback* Callback, uint16_t len )
  {
    Transaction_t Transaction;
    if(pBuffer == nullptr) return W25QXX_ERROR;
...
#if 1   
    m_SpiDriver->XferDMA(&Transaction);   
#else
     m_SpiDriver->TxDMA(pBuffer,len+4,Callback);
#endif    
    return W25QXX_SUCCESS;
  }  
  ...
  w25qxx::Status_t w25qxx::PageReadDMA(uint32_t Page_Address, uint8_t* pBuffer, HAL::Spi::spiCallback* Callback, uint16_t len )
  {
    Transaction_t Transaction;
    if(pBuffer == nullptr) return W25QXX_ERROR;
#if 1
...
//    if(Callback) m_CurrentCallback = Callback; 
    ...
    m_SpiDriver->XferDMA(&Transaction);    
#else
    m_SpiDriver->TxPoll(pBuffer,5);
    m_SpiDriver->RxDMA(pBuffer,len,Callback);
#endif    
    return W25QXX_SUCCESS;
  }  
}

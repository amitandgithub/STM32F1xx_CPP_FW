...
#define PRINTF_FLOAT_SUPPORT // + 1300 bytes with "double", + 960 bytes with "float"

#define FLOAT_AFTER_POINT 4
...
//#define PRINTF_LONG_SUPPORT // + 144 bytes

/* Enable long long int support (implies long int support) */
//#define PRINTF_LONG_LONG_SUPPORT  // + 836 bytes
...
# endif
            ...
            case 'f':
#if 1
              ftoa(va_arg(va, float), p.bf, FLOAT_AFTER_POINT);
#else
              ...
              fpart = fpart * pow10[FLOAT_AFTER_POINT];
              p.bf = &p.bf[i + 1];
              i2a((int)fpart, &p);
              p.width = i + 1 + _strnlen_s(&p.bf[i + 1],10);
              p.bf = bf;
#endif
                putchw(putp, putf, &p);
                break;

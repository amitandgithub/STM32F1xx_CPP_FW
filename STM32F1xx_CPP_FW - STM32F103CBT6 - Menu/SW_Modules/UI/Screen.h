...
namespace HMI
{
  ...
  public:
    ...
    Screen():Window(nullptr),m_CurrentActiveWindow(0),m_RegisteredWindowsIndex(0),m_Captured(0)
    {
    ...
    void EventHandler(InputEvents_t InputEvents)
    {
      if(InputEvents == HMI::UP)
      {
        if(m_Captured)
        {
          m_Windows[m_CurrentActiveWindow]->Run(HMI::UP);
        }
        else
        {
      ...
      else if(InputEvents == HMI::DOWN)
      {
        if(m_Captured)
        {
          m_Windows[m_CurrentActiveWindow]->Run(HMI::DOWN);
        }
        else
        {
          m_Windows[m_CurrentActiveWindow]->Deselect(BLACK);
          m_CurrentActiveWindow++;          
          if(m_CurrentActiveWindow >= MAX_WINDOWS)  m_CurrentActiveWindow = 0; 
          m_Windows[m_CurrentActiveWindow]->Select(WINDOW_HIGHLIGHT_COLOR);
        }             
      }
      else if(InputEvents == HMI::PRESS)
      {
        if(m_Captured)
        {
          m_Captured = m_Windows[m_CurrentActiveWindow]->Run(HMI::PRESS);
        }
        else
        {
          m_Captured = 1;
          //SelectWindow(m_CurrentActiveWindow);
        }       
      }
      else if(InputEvents == HMI::LONGPRESS)
      {
        if(m_Captured)
        {
          m_Captured = m_Windows[m_CurrentActiveWindow]->Run(HMI::LONGPRESS);
        }
        else
        {
          m_Captured = 0;

...
case I2C_INT_TX_1_RX_1_TXN:
    ...
    case I2C_INT_TX_QUEUE:
      ...
      Transaction.RepeatedStart = RepeatedStart;
      Transaction.XferDoneCallback = &I2CCallback;            
      Status = i2c1.Post(&Transaction);
      while(i2c1.Post(&Transaction) != I2c::I2C_TXN_QUEUE_ERROR)
      {
        Posts++;
      }      
    ...
    case I2C_DMA_TX_QUEUE:
      ...
      Transaction.RepeatedStart = RepeatedStart;
      Transaction.XferDoneCallback = &I2CCallback;            
      Status = i2c1.Post(&Transaction,1);
      while(i2c1.Post(&Transaction,1) != I2c::I2C_TXN_QUEUE_ERROR)
      {
        Posts++;
      }      
      while(I2CCallback.get_XferComplete() < (count+Posts))
...
void I2c_Full_EEPROM_POLL()
{
  ...
  {    
    TxBuf[0]  = block;
    i2c1.XferPoll(SlaveAddress,TxBuf,TxLen+2);
    i2c1.XferPoll(SlaveAddress,TxBuf,2,RxBuf,RxLen); 
    LL_mDelay(AT24C128_READ_WRITE_DELAY);
    Test_Condition(!(std::memcmp( (const void*) &TxBuf[2],(const void*) RxBuf, TxLen )), STR("I2C_POLL_FULL_EEPROM = Pass"), STR("I2C_POLL_FULL_EEPROM = Fail"));
    I2C_PRINT("Block = %d \n",block);
  }
}

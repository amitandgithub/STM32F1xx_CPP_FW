...
{ 
  ...
  public:
    ...
    void BaseHwInit(LL_TIM_InitTypeDef* pTIM_InitStruct );
      
    TimerStatus_t HwInit(LL_TIM_InitTypeDef* pTIM_InitStruct = nullptr);
    ...
    void EnableTimerCounter(){LL_TIM_EnableCounter(m_TIMx);}
    
    void DisableTimerCounter(){LL_TIM_DisableCounter(m_TIMx);}
   ...
   void PulseIn_HwInit(Chanel_t channel, uint8_t* Buf, uint16_t len, LL_TIM_InitTypeDef* pLL_TIM_InitStruct = nullptr);

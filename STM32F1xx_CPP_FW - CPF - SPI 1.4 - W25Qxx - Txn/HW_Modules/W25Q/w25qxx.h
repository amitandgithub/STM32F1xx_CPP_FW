...
#define w25qxx_TEMPLATE_T pSpiDriver,CSPort,CSPin
  ...
  template<w25qxx_TEMPLATE_PARAMS>
    ...
    using Callback_t = HAL::Spi::SPICallback_t;
      ...
      Status_t PageWrite(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback = nullptr);
 ...
 //###################################################################################################################
  template<w25qxx_TEMPLATE_PARAMS>
    ...
    Status_t w25qxx<w25qxx_TEMPLATE_T>::PageWrite(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback )
    {
...
//      pBuffer[3] = (Page_Address & 0xFF);
  ...
  template<w25qxx_TEMPLATE_PARAMS> 
    w25qxx<w25qxx_TEMPLATE_T>::
    Status_t w25qxx<w25qxx_TEMPLATE_T>::PageRead(uint32_t Page_Address, uint8_t* pBuffer, HAL::Callback* Callback )
    {
      Transaction_t Transaction;
      if(pBuffer == nullptr) return W25QXX_ERROR;
      ...
      Transaction.TxBuf = pBuffer;
      Transaction.TxLen = 5;
      Transaction.RxBuf = pBuffer + 5;
      Transaction.RxLen = 256;
      Transaction.XferDoneCallback = &w25qxxCb;

...
BSP::HwButton<DigitalIn<B9,INPUT_PULLDOWN>,
  reinterpret_cast <Callback *>(&ClickCb),
  reinterpret_cast <Callback *>(&LongPressCb),
  reinterpret_cast <Callback *>(&LongLongPressCb)> B9_HwBtnInt; 
...
{    
  ...
  char arr[25];
  HAL::Rtc::RtcTime_t aRtcTime;
  
  rtc.GetTime(&aRtcTime);
  sprintf(arr,"T = %02d:%02d:%02d  ", aRtcTime.Hours,aRtcTime.Minutes,aRtcTime.Seconds);
  // HomeScreen.DrawStr(DATE_ROW,0,TimeStr);
  
  HomeScreen.DrawStr(TIMER_ROW,0,arr);
  
  B9_HwBtnInt.Run();    
  
  INA219_Dev.Run(&Power);
  
  //len_I = sprintf(arr,"%04f         ", 5.89);
  
  len_I = ftoa(Power.Current, &arr[0], 1);
  HomeScreen.DrawStr(I_ROW,4,&arr[0],len_I);
  HomeScreen.DrawStr(I_ROW, 4 + len_I, "mA             ",10 - len_I );
  arr[len_I++] = ',';
  
  len_V = ftoa(Power.Voltage, &arr[len_I], 2);
  HomeScreen.DrawStr(V_ROW,4,&arr[len_I],len_V);
  HomeScreen.DrawStr(V_ROW, 4 + len_V, "V             ",10 - len_V);
  arr[len_I + len_V++] = ',';
  
  len_C = ftoa(Power.mAH, &arr[len_I + len_V], 2);
  HomeScreen.DrawStr(C_ROW,4,&arr[len_I + len_V]);
  HomeScreen.DrawStr(C_ROW, 4 + len_C, "mAH           ",10 - len_C);    
  arr[len_I + len_V + len_C++] = ',';
  
  len_W = ftoa(mWH, &arr[len_I + len_V + len_C], 2);
  HomeScreen.DrawStr(W_ROW,4,&arr[len_I + len_V + len_C]);
  HomeScreen.DrawStr(W_ROW, 4 + len_W, "WH");    
  arr[len_I + len_V + len_C + len_W++] = '\n';
...
#if SD_CARD
  RUN_EVERY_MILLIS(1000,Save_To_SD_Card(FileName,&arr[0],len_I + len_V + len_C + len_W));
#endif
}

...
extern HAL::GpioOutput TglPin;
namespace HAL
{
  
  template< uint32_t Timerx, TimerChanel_t Chanel, uint32_t FirstEdge, uint32_t SecondEdge, PulseBuf_t* PulseBuf >
    ...
    public:     
      ...
      PulseIn()
      {
        static_assert( (PulseBuf != 0), "Error: PulseBuf can't be NULL");
      }
    ...
    template< uint32_t Timerx, TimerChanel_t Chanel, uint32_t FirstEdge, uint32_t SecondEdge, PulseBuf_t* PulseBuf >
      Timer::TimerStatus_t PulseIn<Timerx,Chanel,FirstEdge,SecondEdge,PulseBuf>::HwInit(uint32_t Prescaler, uint32_t Autoreload, bool CountUp)
      {
    ...
    template< uint32_t Timerx, TimerChanel_t Chanel, uint32_t FirstEdge, uint32_t SecondEdge, PulseBuf_t* PulseBuf >
      void PulseIn<Timerx,Chanel,FirstEdge,SecondEdge,PulseBuf>::ISR()
      {      
        
        if(LL_TIM_IsActiveFlag_CC1((TIM_TypeDef*)Timerx))
        {            
          if(PulseBuf->Idx < (PulseBuf->Len) )
          {
            ICValue1 = LL_TIM_OC_GetCompareCH1((TIM_TypeDef*)Timerx);//            
            PulseBuf->Buf[PulseBuf->Idx++] = (ICValue1 > ICValue2) ? ICValue1 - ICValue2 : MAX_COUNT - ICValue2 + ICValue1; 
          }
          else
          {
            if(PulseBuf->pCallback) PulseBuf->pCallback->CallbackFunction();
          }
        ...
        {         
          if(PulseBuf->Idx < (PulseBuf->Len))
          {
            ...
            PulseBuf->Buf[PulseBuf->Idx++] = (ICValue2 > ICValue1) ? ICValue2 - ICValue1 : MAX_COUNT - ICValue1 + ICValue2;  
          }
          else
          {
            if(PulseBuf->pCallback) PulseBuf->pCallback->CallbackFunction();

...
namespace HAL
{
    ...
    I2c::I2CStatus_t I2c::HwInit(I2Cx_t I2Cx, I2cPins_t I2cPins)
    {        	
      ...
      I2C_InitStruct.ClockSpeed      = LL_I2C_MAX_SPEED_STANDARD;
      ...
      if(I2cPins == I2C1_B6_B7)
      {
        PinsHwInit(B6,B7);
        LL_APB1_GRP1_EnableClock(LL_APB1_GRP1_PERIPH_I2C1);
      }
      else if(I2cPins == I2C2_B10_B11)
      {
        PinsHwInit(B6,B7);
        LL_APB1_GRP1_EnableClock(LL_APB1_GRP1_PERIPH_I2C2);
      }
      else if(I2cPins == I2C1_B8_B9)
      {
      ...
      LL_I2C_Init((I2Cx_t)I2Cx, &I2C_InitStruct);
    ...
    void I2c::PinsHwInit(Port_t SclPort, PIN_t SclPin, Port_t SdaPort, PIN_t SdaPin)
    {      
      DigitalIO::SetOutputMode(SclPort,SclPin,OUTPUT_AF_OPEN_DRAIN,LL_GPIO_SPEED_FREQ_HIGH); // igitalOut<B6,OUTPUT_AF_OPEN_DRAIN> scl;
      DigitalIO::SetOutputMode(SdaPort,SdaPin,OUTPUT_AF_OPEN_DRAIN,LL_GPIO_SPEED_FREQ_HIGH); // DigitalOut<B7,OUTPUT_AF_OPEN_DRAIN> sda;
    }
    
    I2c::I2CStatus_t I2c::CheckAndLoadTxn(I2Cx_t I2Cx, Transaction_t const *pTransaction)
    {
      ...
      if (WAIT_FOR_BUSY_FLAG_TO_CLEAR(I2Cx,I2C_TIMEOUT))
      {
    ...
    {            
      ...
      {              
        ...
        {
          /* Generate Stop */
          I2C_SET_STOPF(I2Cx,StopFlag);
          
          if (WAIT_FOR_STOP_FLAG_TO_CLEAR(I2Cx,I2C_TIMEOUT))
          {
    ...
    void I2c::ScanBus(I2Cx_t I2Cx, uint8_t* pFoundDevices, uint8_t size)
    {
      uint8_t slave, i=0;
      
      for(slave = 0; slave < 255; slave++)
      {
        if(SendSlaveAddress(I2Cx, slave & I2C_DIR_WRITE,0) == I2C_OK)
        {
    ...
    I2c::I2CStatus_t I2c::SendSlaveAddress(I2Cx_t I2Cx, uint16_t SlaveAddress, uint8_t Repeatedstart)
    {
      ...
      I2C_ENABLE_ACK_AND_START(I2Cx);
      
      /* Wait while SB flag is 0 */
      if( WAIT_FOR_SB_FLAG_TO_SET(I2Cx,I2C_TIMEOUT))
      {         
      ...
      I2C_DATA_REG(I2Cx) = SlaveAddress;
      
      /* Wait while ADDR flag is 0 */
      if(WAIT_FOR_ADDR_FLAG_TO_SET(I2Cx,I2C_TIMEOUT) )
      {
        /* if there is ack fail, clear the AF flag */
        if(I2C_EVENT_AF(I2Cx->SR1))
        {
          I2C_CLEAR_AF(I2Cx);
    ...
    I2c::I2CStatus_t I2c::SlaveStartListening(i2cBuf_t* TxBuf, i2cBuf_t* RxBuf )
    {
      ...
      I2C_ENABLE_INT_EVT_BUF_ERR((I2Cx_t)I2Cx);
      
      I2C_ENABLE_ACK((I2Cx_t)I2Cx);
    ...
    I2c::I2CStatus_t I2c::SlaveStartListening_DMA(i2cBuf_t* TxBuf, i2cBuf_t* RxBuf )
    {
      ...
      /* Disable Pos */
      I2C_DISABLE_POS((I2Cx_t)I2Cx);      
      
      /* Enable Last DMA bit */
      I2C_DMA_LAST_BIT_ENABLE(m_(I2Cx_t)I2Cx);     
      
      I2C_ENABLE_INT_EVT_ERR(m_(I2Cx_t)I2Cx); // Buf interrupt is not nedded in DMA mode
      
      I2C_ENABLE_ACK(m_(I2Cx_t)I2Cx);
      
      m_I2CState = I2C_READY;
      
      /* Enable DMA Request */
      I2C_DMA_ENABLE(m_(I2Cx_t)I2Cx);
    ...
    {            
      ...
      {              
        // From here we don't expect buffer interrupts till SB,ADDR is handled
        I2C_DISABLE_INT_BUF(m_(I2Cx_t)I2Cx);
        
        // Rx is pending, generate start or repeated start
        if(m_Transaction.RepeatedStart)
        {
          /* Generate Start */
          I2C_GENERATE_START(m_(I2Cx_t)I2Cx);
        ...
        {
          /* Generate Stop */
          I2C_SET_STOPF(m_(I2Cx_t)I2Cx,StopFlag);
          
          if (WAIT_FOR_STOP_FLAG_TO_CLEAR(m_(I2Cx_t)I2Cx,I2C_TIMEOUT))
          {
            m_I2CStatus = I2C_STOP_TIMEOUT;
            I2C_LOG_STATES(I2C_LOG_STOP_TIMEOUT);  
            while(1);
    ...
    I2c::I2CStatus_t I2c::XferPoll(uint16_t SlaveAddress,uint8_t* TxBuf, uint32_t TxLen, uint8_t* RxBuf, uint32_t RxLen, uint8_t RepeatedStart)
    {
      ...
      if(TxLen != 0)
      {
        ...
        /* Clear ADDR flag */
        I2C_CLEAR_ADDR(m_(I2Cx_t)I2Cx);
        
        while(TxLen)
        {    
          /* Write data to DR */
          I2C_DATA_REG(m_(I2Cx_t)I2Cx) = *TxBuf++;                
          TxLen--; 
          
          /* Wait until TXE flag is set */
          if(WAIT_FOR_TXE_FLAG_TO_SET(m_(I2Cx_t)I2Cx,I2C_TIMEOUT))
          {          
          ...
          if(I2C_EVENT_BTF(m_(I2Cx_t)I2Cx->SR1))
          {
        ...
        if(!RepeatedStart)
        {
          I2C_GENERATE_STOP(m_(I2Cx_t)I2Cx);
      ...
      if(RxLen != 0)
      {
        ...
        {
          /* Disable Acknowledge */
          I2C_DISABLE_ACK(m_(I2Cx_t)I2Cx);
        ...
        while(RxLen > 0U)
        {
          if(RxLen <= 3U)
          {
            ...
            if(RxLen == 1U)
            {
              ...
              if(WAIT_FOR_RXNE_FLAG_TO_SET(m_(I2Cx_t)I2Cx,I2C_TIMEOUT))
              {          
              ...
              /* Read data from DR */
              (*RxBuf) = I2C_DATA_REG(m_(I2Cx_t)I2Cx);
              RxLen--;
            ...
            else if(RxLen == 2U)
            {
              ...
              if(WAIT_FOR_BTF_FLAG_TO_SET(m_(I2Cx_t)I2Cx,I2C_TIMEOUT))
              {         
    ...
    I2c::I2CStatus_t I2c::XferIntr(Transaction_t const *pTransaction)  // 178 bytes
    {
      ...
      I2C_ENABLE_INT_EVT_BUF_ERR(m_(I2Cx_t)I2Cx);
      
      /* Enable Acknowledge, Generate Start */
      I2C_ENABLE_ACK_AND_START(m_(I2Cx_t)I2Cx);  
...
#pragma inline = forced
    ...
    {      
      ...
      if(m_I2CTxnQueue.AvailableEnteries())
      {
        ...
        /* Disable Pos */
        I2C_DISABLE_POS(m_(I2Cx_t)I2Cx);
      ...
      {        
        ...
        {
          I2C_DISABLE_INT_EVT_BUF_ERR(m_(I2Cx_t)I2Cx); 
    ...
    {     
      ...
      I2C_ENABLE_INT_EVT_ERR(m_(I2Cx_t)I2Cx);
...
#pragma inline = forced
    ...
    {      
      ...
      {        
        ...
        else
        {
          ...
          /* Disable Last DMA */
          I2C_DMA_LAST_BIT_DISABLE(m_(I2Cx_t)I2Cx); 
          
          /* Disable DMA Request */            
          I2C_DMA_DISABLE(m_(I2Cx_t)I2Cx);               
    ...
    void I2c::ISR()
    {  
      switch(I2C_GET_EVENT(m_(I2Cx_t)I2Cx))
      {
      case EVENT_SB :
        I2C_LOG_EVENTS(I2C_LOG_SB);
        if( (m_I2CState == I2C_MASTER_TX) || (m_I2CState == I2C_MASTER_TX_DMA) )
        {
          I2C_DATA_REG(m_(I2Cx_t)I2Cx) = m_Transaction.SlaveAddress & I2C_DIR_WRITE; 
        ...
        else if((m_I2CState == I2C_MASTER_RX) || (m_I2CState == I2C_MASTER_RX_DMA))  
        {
          ...
          if((m_I2CState == I2C_MASTER_RX))
            I2C_ENABLE_INT_BUF(m_(I2Cx_t)I2Cx);
...
#endif 
          I2C_DATA_REG(m_(I2Cx_t)I2Cx) = m_Transaction.SlaveAddress | I2C_DIR_READ;
        ...
        else if(m_I2CState == I2C_MASTER_RX_REPEATED_START)
        {
          ...
          if(m_(I2Cx_t)I2Cx->CR2 & I2C_CR2_DMAEN)
          {
            m_I2CState = I2C_MASTER_RX_DMA;
      ...
      case EVENT_ADDR: 
        ...
        if(m_I2CState == I2C_MASTER_RX)
        {
          ...
          {
            /* Clear ADDR flag */
            LL_I2C_ClearFlag_ADDR(m_(I2Cx_t)I2Cx);
        ...
        else if((m_I2CState == I2C_READY) || (m_I2CState == I2C_SLAVE_RX) || (m_I2CState == I2C_SLAVE_TX) || (m_I2CState == I2C_SLAVE_RX_DMA) || (m_I2CState == I2C_SLAVE_TX_DMA)) 
        {
          ...
          if( (m_(I2Cx_t)I2Cx->SR2 & LL_I2C_SR2_DUALF) == RESET)
          {
            m_Transaction.SlaveAddress = (m_(I2Cx_t)I2Cx->OAR1 & 0x00FE); // Bit 1-7 are address
          }
          else
          {
            m_Transaction.SlaveAddress = (m_(I2Cx_t)I2Cx->OAR2 & 0x00FE); // Bit 1-7 are address
          }            
          /* Transfer Direction requested by Master */
          if( (m_(I2Cx_t)I2Cx->SR2 & LL_I2C_SR2_TRA) == 0)
          {
      ...
      case EVENT_BTF : 
        ...
        else if(m_I2CState == I2C_SLAVE_RX ) 
        {
          ...
          {     
            ...
            I2C_DATA_REG(m_(I2Cx_t)I2Cx) = I2C_DATA_REG(m_(I2Cx_t)I2Cx);
        ...
        else if(m_I2CState == I2C_SLAVE_TX )    
        {
          ...
          else
          {
            ...
            I2C_DATA_REG(m_(I2Cx_t)I2Cx) = m_SlaveTxn.DefaultByte;  
          }      
        }
        else
        {
      ...
      case EVENT_STOPF : 
        ...
        I2C_CLEAR_STOPF(m_(I2Cx_t)I2Cx);  
      ...
      case EVENT_BERR : 
        ...
        LL_I2C_ClearFlag_BERR(m_(I2Cx_t)I2Cx);
      ...
      case EVENT_ARLO : 
        ...
        LL_I2C_ClearFlag_ARLO(m_(I2Cx_t)I2Cx);
      ...
      case EVENT_AF : // NACK        
        ...
        LL_I2C_ClearFlag_AF(m_(I2Cx_t)I2Cx); 
        if((m_I2CState == I2C_MASTER_TX) || (m_I2CState == I2C_MASTER_TX_DMA)) 
        {
          /* Generate Stop */
          I2C_GENERATE_STOP(m_(I2Cx_t)I2Cx);
      ...
      case EVENT_OVR : 
        ...
        LL_I2C_ClearFlag_OVR(m_(I2Cx_t)I2Cx);
    ...
    {  
      ...
      case EVENT_ADDR: 
        ...
        else if((m_I2CState == I2C_READY) || (m_I2CState == I2C_SLAVE_RX_DMA) || (m_I2CState == I2C_MASTER_TX_DMA))    
        {
          ...
          /* Transfer Direction requested by Master */
          if( (m_(I2Cx_t)I2Cx->SR2 & LL_I2C_SR2_TRA) == RESET)
          {
            m_I2CState = I2C_SLAVE_RX_DMA;               
          }
          else

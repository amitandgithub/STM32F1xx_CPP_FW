...
//I2c i2cX(Gpio::B6, Gpio::B7,100000U);
...
extern HAL::I2cMasterIntr<I2c::i2c1> i2c_1;
...
void I2c_Tests()
{
  ...
  i2cX.HwInit(HAL::I2c::I2C1_B6_B7);
  
  //i2cX.ScanBus(RxBuf,20);
  SlaveAddress = 0x68<<1;
  I2c_test_id = I2C_POLL_TX_1_RX_1;//I2C_POLL_TX_1_RX_1;
  TxBuf[0] = 0x00;
  TxBuf[1] = 0x00;
#if I2C_POLL
  i2cX.XferPoll(SlaveAddress,TxBuf,2); 
#endif
  TxBuf[0] = 8;
  int Posts=0;
  ...
  while(1)
  {
    ...
    case I2C_POLL_TX_1_RX_1:
      TxLen = RxLen = 1;
      i2cX.XferPoll(SlaveAddress,TxBuf,TxLen+1);
      i2cX.XferPoll(SlaveAddress,TxBuf,2,RxBuf,RxLen,RepeatedStart); 
    ...
    case I2C_POLL_40_RX_40: 
      TxLen = RxLen = 40;           
      i2cX.XferPoll(SlaveAddress,TxBuf,TxLen+1);
      i2cX.XferPoll(SlaveAddress,TxBuf,1,RxBuf,RxLen,RepeatedStart);            
    ...
    case I2C_POLL_TX_3_RX_3_AT24C128:
      ...
      TxBuf_AT24C128[0] = 0;
       TxBuf_AT24C128[1] = 0;
       TxBuf_AT24C128[5] += 1;
      i2cX.XferPoll(SlaveAddress,TxBuf_AT24C128,TxLen);
       LL_mDelay(10);
      i2cX.XferPoll(SlaveAddress,TxBuf_AT24C128,2,RxBuf,RxLen,RepeatedStart);
...
case I2C_INT_TX_1_RX_1_TXN:
      ...
      i2cX.Xfer(&Transaction);
      while(i2cX.GetState() != HAL::I2c::I2C_READY); 
...
#endif
#if I2C_MASTER_Q && I2C_MASTER_INTR
    ...
    case I2C_INT_TX_QUEUE:
      ...
      Transaction.RepeatedStart = RepeatedStart;
      Transaction.XferDoneCallback = &I2CCallback;            
      Status = i2cX.Post(&Transaction);
      while(i2cX.Post(&Transaction) != I2c::I2C_TXN_QUEUE_ERROR)
      {
        Posts++;
      }      
      while(I2CCallback.get_XferComplete() < (Callbackcount+Posts))
      {
        I2C_PRINT("Q posts = %d \t ISR Count = %d \n",Posts, (Callbackcount+Posts));
      }
      
      Test_Condition( 1, STR("I2C_INT_TX_QUEUE = Pass"), STR("I2C_INT_TX_QUEUE = Fail"));
      
#endif //I2C_MASTER_Q && I2C_MASTER_INTR
#if I2C_MASTER_DMA
...
#if I2C_MASTER_Q && I2C_MASTER_DMA
      
      I2c_test_id = I2C_DMA_TX_QUEUE;			
      break;
    case I2C_DMA_TX_QUEUE:
      ...
      Transaction.RepeatedStart = RepeatedStart;
      Transaction.XferDoneCallback = &I2CCallback;            
      Status = i2cX.Post(&Transaction,1);
      while(i2cX.Post(&Transaction,1) != I2c::I2C_TXN_QUEUE_ERROR)
      {
        Posts++;
      }      
      while(I2CCallback.get_XferComplete() < (Callbackcount+Posts))
      ...
      Test_Condition( 1, STR("I2C_DMA_TX_QUEUE = Pass"), STR("I2C_DMA_TX_QUEUE = Fail"));
      I2c_test_id = 200;
#endif //I2C_MASTER_Q && I2C_MASTER_DMA
    default:         
      if(RepeatedStart == 0)
      {
...
{  
  ...
  i2cX.SetSlaveTxDefaultByte(0xff);
  
  i2cX.SetSlaveCallback(&I2C_Slave_DMA_Callback);
    
  while(i2cX.SlaveStartListening_DMA(&I2C_Slave_TxBuf, &I2C_Slave_RxBuf) != HAL::I2c::I2C_OK);
}
...
void I2c_Tests_AT24C128()
{
  ...
  while(1)
  {
    ...
    case I2C_POLL_TX_1_RX_1:
      TxLen = RxLen = 1;
      i2cX.XferPoll(SlaveAddress,TxBuf,TxLen+2);
...
//  {    
//    TxBuf[0]  = block;
//    i2cX.XferPoll(0xA0,TxBuf,TxLen+2);
//    LL_mDelay(AT24C128_READ_WRITE_DELAY);
//    
//    if(block%17 == 0)
//    {
...
//    MemSet(RxBuf,0,sizeof(RxBuf));
//     LL_mDelay(AT24C128_READ_WRITE_DELAY);
//    i2cX.XferPoll(0xA0,TxBuf,2,RxBuf,RxLen); 
//    
//    
